...
flowchart TB
 subgraph Root["🔸 프로젝트 루트"]
    direction LR
        Config[".env<br>환경 변수"]
        Main["main.py<br>진입점"]
        Req["requirements.txt<br>의존성"]
  end
 subgraph Configs["🔸 설정 (configs/)"]
    direction TB
        DBConfig["db_config.yaml<br>DB 설정"]
        ModelConfig["model_config.yaml<br>모델 설정"]
        MultiRequest["multi_request_patterns.yaml<br>다중 요청 패턴"]
        PromptConfig["prompt_config.yaml<br>프롬프트 설정"]
        CollectConfig["collect/config.yaml<br>데이터 수집 설정"]
  end
 subgraph SourceCore["🔷 핵심 로직"]
    direction LR
        Tools["tools/<br>도구 시스템"]
        Agent["agent/<br>AI Agent<br>Fallback Chain"]
        PromptsMgr["prompts/<br>프롬프트 관리"]
  end
 subgraph SourceData["🔶 데이터 처리"]
    direction LR
        Papers["papers/<br>논문 처리"]
        RAG["rag/<br>RAG 검색"]
        DataLoader["data/<br>문서 로더"]
  end
 subgraph SourceInfra["🔺 인프라"]
    direction LR
        DB["database/<br>DB 연결"]
        LLM["llm/<br>LLM 클라이언트"]
        Text2SQL["text2sql/<br>SQL 변환"]
  end
 subgraph SourceSupport["💡 지원 기능"]
    direction LR
        Eval["evaluation/<br>성능 평가"]
        Utils["utils/<br>유틸리티"]
        Mem["memory/<br>대화 메모리"]
  end
 ...
 subgraph Prompts["🔺 프롬프트 엔지니어링<br>(prompts/)"]
    direction TB
        RoutingPrompts["routing_prompts.json<br>라우팅 프롬프트"]
        ToolPrompts["tool_prompts.json<br>도구 프롬프트"]
        EvalPrompts["evaluation_prompts.json<br>평가 프롬프트"]
        QGenPrompts["question_generation_prompts.json<br>질문 생성 프롬프트"]
        GoldenDataset["golden_dataset.json<br>골든 데이터셋"]
  end
 subgraph UI["🔺 사용자 인터페이스 (ui/)"]
    direction LR
        Comp["components/<br>UI 컴포넌트"]
        App["app.py<br>메인 UI"]
        Pages["pages/<br>추가 페이지"]
  end
 subgraph Scripts["🔶 스크립트 (scripts/)"]
    direction LR
        System["system/<br>시스템 관리"]
        DataScript["data/<br>데이터 처리"]
        Tests["tests/<br>테스트"]
  end
 subgraph DocsBasic["📑 기본 문서"]
    direction LR
        PRD2["PRD/<br>요구사항"]
        Rules["rules/<br>규칙"]
        Roles["roles/<br>담당역할"]
        Minutes["minutes/<br>회의록"]
  end
 subgraph DocsTracking["📊 추적 문서"]
    direction LR
        Errors["errors/<br>에러"]
        Issues["issues/<br>이슈 트래킹"]
        Experiments["experiments/<br>실험 보고서"]
  end
 subgraph DocsArch["🏗️ 아키텍처 문서"]
    direction LR
        Arch["architecture/<br>단일요청/다중요청/<br>멀티턴 기능별<br>아키텍처"]
        Mod["modularization/<br>모듈 문서"]
  end
 subgraph DocsUsage["📖 사용 가이드"]
    direction LR
        QnA["QnA/<br>프로젝트 관련<br>질의응답"]
        Usage["usage/<br>실행 명령어"]
        Scenarios["scenarios/<br>질문 리스트<br>시나리오"]
  end
 ...
 subgraph MainFlow["📋 프로젝트 전체 구조"]
    ...
    Main --> Config
    Config --> Req
    DBConfig ~~~ ModelConfig
    ModelConfig ~~~ MultiRequest
    MultiRequest ~~~ PromptConfig
    PromptConfig ~~~ CollectConfig
    Agent --> Tools
    Tools --> PromptsMgr
    RAG --> Papers
    Papers --> DataLoader
    ...
    RoutingPrompts ~~~ ToolPrompts
    ToolPrompts ~~~ EvalPrompts
    EvalPrompts ~~~ QGenPrompts
    QGenPrompts --- GoldenDataset
    App --> Comp
    Comp --> Pages
    DataScript --> System
    System --> Tests
    ...
    style DBConfig fill:transparent,stroke:#f57c00,stroke-width:2px,color:#000
    style ModelConfig fill:transparent,stroke:#f57c00,stroke-width:2px,color:#000
    style MultiRequest fill:transparent,stroke:#f57c00,stroke-width:2px,color:#000
    style PromptConfig fill:transparent,stroke:#f57c00,stroke-width:2px,color:#000
    style CollectConfig fill:transparent,stroke:#f57c00,stroke-width:2px,color:#000
    ...
    style SourceCore fill:#E9BEE7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style SourceData fill:#e9AEE7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style SourceInfra fill:#daCaf2,stroke:#7b1fa2,stroke-width:2px,color:#000
    style SourceSupport fill:#d5ACf7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style RoutingPrompts fill:transparent,stroke:#01579b,stroke-width:2px,color:#000
    style ToolPrompts fill:transparent,stroke:#01579b,stroke-width:2px,color:#000
    style EvalPrompts fill:transparent,stroke:#01579b,stroke-width:2px,color:#000
    style QGenPrompts fill:transparent,stroke:#01579b,stroke-width:2px,color:#000
    style GoldenDataset fill:transparent,stroke:#01579b,stroke-width:2px,color:#000
    ...
    style PRD2 fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Rules fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Roles fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Minutes fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Errors fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Issues fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Experiments fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000
    style Arch fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000000
    style Mod fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000000
    style QnA fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000000
    style Usage fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000000
    style Scenarios fill:#f8bbd0,stroke:#880e4f,stroke-width:2px,color:#000000
    style DocsBasic fill:#FFCDD2,stroke:#880e4f,stroke-width:2px,color:#000
    style DocsTracking fill:#f48fb1,stroke:#880e4f,stroke-width:2px,color:#000
    style DocsArch fill:#ff79b0,stroke:#880e4f,stroke-width:2px,color:#fff
    style DocsUsage fill:#f06292,stroke:#880e4f,stroke-width:2px,color:#fff
    ...
    linkStyle 2 stroke:#f57c00,stroke-width:2px,fill:none
    linkStyle 3 stroke:#f57c00,stroke-width:2px,fill:none
    linkStyle 4 stroke:#f57c00,stroke-width:2px,fill:none
    linkStyle 5 stroke:#f57c00,stroke-width:2px,fill:none
    linkStyle 6 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 7 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 8 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 9 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 10 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 11 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 12 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 13 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 14 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 15 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 16 stroke:#7b1fa2,stroke-width:2px,fill:none
    linkStyle 17 stroke:#01579b,stroke-width:2px,fill:none
    linkStyle 18 stroke:#01579b,stroke-width:2px,fill:none
    linkStyle 19 stroke:#01579b,stroke-width:2px,fill:none
    linkStyle 20 stroke:#01579b,stroke-width:2px,fill:none
    linkStyle 21 stroke:#2e7d32,stroke-width:2px,fill:none
    linkStyle 22 stroke:#2e7d32,stroke-width:2px,fill:none
    linkStyle 23 stroke:#e65100,stroke-width:2px,fill:none
    linkStyle 24 stroke:#e65100,stroke-width:2px,fill:none
    linkStyle 25 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 26 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 27 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 28 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 29 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 30 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 31 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 32 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 33 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 34 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 35 stroke:#880e4f,stroke-width:2px,fill:none
    linkStyle 36 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 37 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 38 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 39 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 40 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 41 stroke:#616161,stroke-width:3px,fill:none
    linkStyle 42 stroke:#616161,stroke-width:3px,fill:none
```
...
graph TB
    subgraph AgentDetail["🤖 Agent 실행 내부 구조 (LangGraph)"]
        ...
        subgraph RouterPhase["🔸 라우팅 단계"]
            direction LR
            RouterStart[라우터<br/>시작] --> MultiCheck{다중요청<br/>패턴?}
            MultiCheck -->|예| MultiRoute[패턴 매칭<br/>YAML 파일]
            MultiCheck -->|아니오| SingleRoute[단일요청<br/>처리]
            MultiRoute --> SetPipeline[도구 파이프라인<br/>설정]
            SingleRoute --> ClassifyQ[질문 유형<br/>분류]
            ClassifyQ --> SetChain[Fallback 체인<br/>설정]
        end

        subgraph ValidationPhase["🔹 검증 단계"]
            direction LR
            Validate{도구 검증<br/>활성화?} -->|예| ValidateNode[도구 선택<br/>타당성 검증]
            Validate -->|아니오| SkipValidate[검증 생략]
            ValidateNode -->|실패| RetryRouter[라우터 재실행<br/>최대 2회]
        ...
        subgraph ToolExecPhase["🔺 도구 실행 단계"]
            direction LR
            ToolNode[선택된 도구<br/>실행] --> WrapCheck{도구 래퍼<br/>적용?}
            WrapCheck -->|예| AutoStatus[도구 상태<br/>자동 설정]
            WrapCheck -->|아니오| ManualStatus[기존 방식<br/>유지]
        ...
        subgraph FailurePhase["🔶 실패 감지 단계"]
            direction LR
            CheckStatus{도구 상태<br/>확인} -->|성공| NextCheck{다음<br/>도구?}
            CheckStatus -->|실패| FailDetect[실패 원인<br/>분석]
            FailDetect --> CountRetry{재시도<br/>횟수?}
            CountRetry -->|재시도 가능| FallbackRoute[Fallback 라우터<br/>다음 도구 선택]
            CountRetry -->|최대 초과| FinalFallback[최종 Fallback<br/>일반 답변 강제]
            NextCheck -->|예| PipelineRouter[파이프라인 라우터<br/>다음 도구]
            NextCheck -->|아니오| Success([✅ 성공<br/>최종 답변])
        end

        subgraph FallbackPhase["💡 Fallback 체인 단계"]
            direction LR
            FallbackStart[Fallback 체인<br/>조회] --> GetNext{다음 도구<br/>존재?}
            GetNext -->|예| NextTool[다음 우선순위<br/>도구 선택]
            GetNext -->|아니오| ForceGeneral[일반 답변<br/>강제 실행]
            NextTool --> UpdateChain[실패 도구<br/>기록 업데이트]
            ForceGeneral --> UpdateChain
            UpdateChain --> RetryTool[선택된 도구<br/>재실행]
        end

        subgraph MultiTurnPhase["🔷 멀티턴 대화"]
            direction LR
            History[대화 히스토리<br/>저장소] --> Context[이전 대화<br/>문맥 추출]
            Context --> ContextRouter[라우터에<br/>문맥 전달]
