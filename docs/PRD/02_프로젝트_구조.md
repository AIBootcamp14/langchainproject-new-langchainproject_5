...
│   │   ├── text2sql.py           # Text-to-SQL 도구 (논문 통계 조회)
...
6. `text2sql.py`: 자연어→SQL 변환하여 논문 통계 조회
7. `file_save.py`: 대화/요약 내용 파일 저장
...
- `config.py`: 모델 설정 (GPT-5, Solar Pro2)
...
- Agent 라우팅 단계에서 질문 유형을 분석하여 일반 답변, RAG, 웹 검색, 용어집, 요약, Text-to-SQL (통계 조회), 저장 중 적절한 도구를 선택 (총 7개 도구)
- 데이터 조회 단계에서 선택된 도구에 따라 LLM 직접 호출, PGVector 검색, PostgreSQL 조회 (용어집/papers 테이블), Tavily API 호출 등을 수행
...
| 순서 | 그룹 | 설명 | 예시 |
|------|------|------|------|
| 1 | 표준 라이브러리 | Python 내장 모듈 | os, sys, datetime |
| 2 | 서드파티 라이브러리 | pip로 설치한 외부 패키지 | numpy, langchain, langchain_openai |
| 3 | 로컬 모듈 | 프로젝트 내부 모듈 (src/*) | src.utils.logger, src.llm.client |

**그룹 간 구분:**
- 각 그룹 사이에 빈 줄 1개 삽입
- 그룹 내에서는 알파벳 순서로 정렬 권장
...
**독스트링 형식**: Google Style Docstring 사용

**함수 독스트링 구조:**

| 섹션 | 필수 여부 | 설명 | 예시 |
|------|----------|------|------|
| 요약 | 필수 | 함수 기능 한 줄 설명 | "논문 데이터베이스에서 관련 논문을 검색합니다." |
| Args | 파라미터 있으면 필수 | 매개변수 설명 (타입, 기본값 포함) | query (str): 검색 질의<br/>k (int): 반환할 문서 개수 (기본값: 5) |
| Returns | 반환값 있으면 필수 | 반환 타입 및 설명 | list: 검색된 Document 객체 리스트 |
| Raises | 예외 발생 시 권장 | 발생 가능한 예외 | ValueError: query가 빈 문자열인 경우 |
| Example | 선택 | 사용 예시 (doctest 형식) | >>> docs = search_paper_database("Transformer", k=3)<br/>>>> print(len(docs))<br/>3 |

**독스트링 작성 규칙:**

| 규칙 | 설명 |
|------|------|
| 위치 | 함수/클래스 정의 바로 다음 줄 (첫 번째 문장) |
| 따옴표 | 삼중 따옴표 (""") 사용 |
| 들여쓰기 | 함수 본문과 동일한 수준 |
| 타입 힌트 | 함수 시그니처에 타입 힌트 추가 (query: str, k: int = 5 등) |
| 예시 형식 | doctest 형식 (>>> 프롬프트 사용) |
