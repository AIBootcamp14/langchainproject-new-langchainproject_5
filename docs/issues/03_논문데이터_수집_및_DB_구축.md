...
- [x] ArxivPaperCollector 클래스 구현 (`scripts/collect_arxiv_papers.py`)
  - [x] arxiv 패키지 사용하여 논문 검색
  - [x] collect_papers() 메서드 (query, max_results 파라미터)
  - [x] PDF 다운로드 및 저장 (data/raw/pdfs/)
  - [x] 메타데이터 추출 (title, authors, publish_date, summary, pdf_url, categories)
  - [x] collect_by_keywords() 메서드 (여러 키워드 반복 수집)
  - [x] remove_duplicates() 메서드 (제목 기준 중복 제거)
- [x] AI/ML 키워드 리스트 정의
  - [x] "transformer attention", "BERT GPT", "large language model"
  - [x] "retrieval augmented generation", "neural machine translation"
  - [x] "question answering", "AI agent"
- [x] 메타데이터 JSON 파일 저장 (data/raw/arxiv_papers_metadata.json)

### Phase 2: Document Loader 구현 (1일)
- [x] PaperDocumentLoader 클래스 구현 (`src/data/document_loader.py`)
  - [x] RecursiveCharacterTextSplitter 초기화 (chunk_size=1000, chunk_overlap=200)
  - [x] load_pdf() 메서드 (PyPDFLoader 사용)
  - [x] load_and_split() 메서드 (PDF → 청크 분할)
  - [x] 각 청크에 chunk_id 메타데이터 추가
  - [x] load_all_pdfs() 메서드 (디렉토리 전체 PDF 처리)
- [x] 메타데이터 매핑 (arXiv ID → 논문 정보)

### Phase 3: 임베딩 및 Vector DB 적재 (1일)
- [x] PaperEmbeddingManager 클래스 구현 (`src/data/embeddings.py`)
  - [x] OpenAI Embeddings 초기화 (text-embedding-3-small)
  - [x] PGVector VectorStore 초기화 (collection: paper_chunks)
  - [x] add_documents() 메서드 (배치 처리, batch_size=50)
  - [x] add_documents_with_paper_id() 메서드 (paper_id 메타데이터 추가)
  - [x] 진행 상황 로깅
  - [x] 오류 처리 (API 속도 제한 대응)
- [x] 배치 처리 로직 (OpenAI API Rate Limit 고려)

### Phase 4: 데이터베이스 초기 설정 (1일)
- [x] PostgreSQL 스키마 생성 (`scripts/setup_database.py`)
  - [x] create_tables() 함수
    - [x] papers 테이블 생성 (paper_id, title, authors, publish_date, url, category, citation_count, abstract)
    - [x] glossary 테이블 생성 (term_id, term, definition, easy_explanation, hard_explanation, category)
    - [x] 인덱스 생성 (papers.title, glossary.term)
  - [x] insert_paper_metadata() 함수
    - [x] JSON 메타데이터를 papers 테이블에 INSERT
    - [x] ON CONFLICT (url) DO NOTHING (중복 방지)
    - [x] RETURNING paper_id로 ID 조회
    - [x] arXiv ID → paper_id 매핑 딕셔너리 생성
  - [x] insert_glossary_data() 함수
    - [x] 초기 용어집 데이터 삽입 (Attention Mechanism, Fine-tuning, BLEU Score 등)
- [x] pgvector extension 설치 및 초기화
- [x] paper_id_mapping.json 저장 (data/processed/)

### Phase 5: 인수인계 문서 작성 (반나절)
- [x] 완료 항목 체크리스트
- [x] 데이터 위치 명시 (PDF, 메타데이터, Vector DB)
- [x] 추가 데이터 수집 방법 가이드
- [x] DB 연결 정보 및 주의사항
- [x] Logger 적용 및 로그 기록
