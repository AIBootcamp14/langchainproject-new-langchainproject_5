...
```mermaid
graph TB
    subgraph Input["1️⃣ 사용자 입력"]
        User[사용자 질문 + 난이도]
    end

    subgraph JSONFiles["2️⃣ JSON 프롬프트 파일"]
        RP[routing_prompts.json<br/>라우팅 프롬프트]
        TP[tool_prompts.json<br/>도구별 프롬프트]
    end

    subgraph PromptLoader["3️⃣ 프롬프트 로더"]
        Loader[src/prompts/loader.py]
        Init[src/prompts/__init__.py]
    end

    subgraph AgentGraph["4️⃣ Agent 그래프"]
        Router[Router Node<br/>도구 선택]

        subgraph ToolNodes["도구 노드 (7개)"]
            General[General Answer<br/>✅ JSON 적용]
            WebSearch[Web Search<br/>✅ JSON 적용]
            Summarize[Summarize<br/>✅ JSON 적용]
            SearchPaper[Search Paper<br/>✅ JSON 적용]
            Glossary[Glossary<br/>✅ JSON 적용]
            SaveFile[Save File<br/>✅ 검증 완료]
            Text2SQL[Text-to-SQL<br/>✅ 통합 완료]
        end
    end

    subgraph LLMCall["5️⃣ LLM 호출"]
        LLM[LLM Client<br/>난이도별 모델 선택]
    end

    subgraph Output["6️⃣ 응답 생성"]
    ...
    classDef loader fill:#90caf9,stroke:#1976d2,stroke-width:2px,color:#000
    classDef applied fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    classDef router fill:#ba68c8,stroke:#7b1fa2,stroke-width:2px,color:#fff
    classDef llm fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#000
    classDef response fill:#66bb6a,stroke:#2e7d32,stroke-width:2px,color:#fff

    class RP,TP jsonFile
    class Loader,Init loader
    class General,WebSearch,Summarize,SearchPaper,Glossary,SaveFile,Text2SQL applied
    class Router router
    class LLM llm
...
graph LR
    ...
    %% 노드 클래스 정의
    classDef difficulty fill:#90caf9,stroke:#1976d2,stroke-width:2px,color:#000
    classDef client fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#fff
