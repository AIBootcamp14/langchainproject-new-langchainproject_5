...
graph TB
    subgraph MainFlow["📋 main.py 실행 워크플로우"]
        ...
        subgraph Streamlit["🔹 Streamlit 서버 시작"]
            direction LR
            RunStreamlit[streamlit run<br/>ui/app.py] --> SetConfig[서버 설정<br/>port: 8501]
        ...
        subgraph AgentSetup["🔶 Agent & ExperimentManager 생성"]
            direction LR
            CreateExpMgr[ExperimentManager<br/>생성] --> CreateGraph[create_agent_graph<br/>LangGraph 구성]
            CreateGraph --> AddNodes[노드 추가<br/>router + 7개 도구]
        ...
        subgraph UserInput["🔷 사용자 입력 처리"]
            direction LR
            InputReceived[질문 입력<br/>수신] --> AddUserMsg[사용자 메시지<br/>추가]
            AddUserMsg --> InvokeAgent[agent_executor.invoke<br/>실행]
        end

        subgraph AgentExec["✨ Agent 실행 (LangGraph)"]
            direction LR
            RouterNode[router_node<br/>라우팅] --> SelectTool{도구<br/>선택}
            SelectTool -->|일반| GeneralNode[general_answer_node]
            SelectTool -->|RAG| SearchNode[search_paper_node]
            SelectTool -->|웹| WebNode[web_search_node]
            SelectTool -->|용어| GlossaryNode[glossary_node]
            SelectTool -->|요약| SummarizeNode[summarize_node]
            SelectTool -->|통계| Text2SQLNode[text2sql_node]
            SelectTool -->|저장| SaveNode[save_file_node]
        end

        subgraph Response["🔸 응답 처리"]
            direction LR
            GetResponse[응답<br/>수신] --> DisplayAnswer[답변<br/>표시]
    ...
    %% 연결선 스타일 (응답 처리 29~31)
    linkStyle 29 stroke:#0288d1,stroke-width:2px
...
### 2. Streamlit 서버 시작
- **streamlit run ui/app.py**: Streamlit 웹 서버 실행
- **서버 설정**:
  - 포트: 8501
...
- **Agent 초기화 (캐싱)**: `@st.cache_resource` 데코레이터로 캐시
  - 날짜별로 캐시 키 생성 (날짜 변경 시 재초기화)

### 4. Agent & ExperimentManager 생성
- **ExperimentManager 생성**: 실험 로그 관리 시스템 초기화
  - 실험 폴더 생성: `experiments/YYYYMMDD/HHMMSS/`
  - 로거 설정: 각 도구별 로그 파일
- **create_agent_graph**: LangGraph 기반 Agent 그래프 구성
- **노드 추가**: router_node + 7개 도구 노드
  - general_answer_node
  - search_paper_node
  - web_search_node
  - glossary_node
  ...
  - text2sql_node
  - save_file_node
- **엣지 추가**: 조건부 라우팅 (route_to_tool, should_continue_pipeline)
- **그래프 컴파일**: StateGraph.compile()
...
- **사이드바 렌더링**: `render_sidebar()`
  ...
  - 새 채팅 시작 버튼
  - 채팅 히스토리 목록
- **채팅 세션 초기화**: `initialize_chat_sessions()`
  - 첫 실행 시 자동으로 새 채팅 생성
- **채팅 히스토리 표시**: `display_chat_history()`
  - 현재 채팅의 모든 메시지 렌더링
- **사용자 입력 대기**: `st.chat_input()` 표시

### 6. 사용자 입력 처리
- **질문 입력 수신**: 사용자가 채팅 입력창에 질문 입력
- **사용자 메시지 추가**: `add_message_to_current_chat()`
  - 세션 상태에 메시지 저장
  - UI에 사용자 채팅 버블 표시
- **agent_executor.invoke()**: Agent 그래프 실행
  - 입력: `{question, difficulty, messages}`
  - 설정: `{callbacks: [StreamlitCallbackHandler]}`

### 7. Agent 실행 (LangGraph)
- **router_node**: 질문 분석 및 라우팅
  - 다중 요청 패턴 매칭 (multi_request_patterns.yaml)
  - 질문 유형 분류 (QuestionClassifier)
  - LLM 라우팅
  - 키워드 매칭 Fallback
- **도구 선택**: route_to_tool() 함수로 다음 노드 결정
  - general_answer_node: LLM 직접 호출
  - search_paper_node: RAG 검색 (PGVector)
  - web_search_node: Tavily API 호출
  - glossary_node: 용어집 검색 (PostgreSQL)
  - summarize_node: 논문 요약
  - text2sql_node: SQL 생성 및 실행
  - save_file_node: 파일 저장
...
- **답변 표시**: `st.chat_message("assistant")`
  - 답변 내용 렌더링
  - 사용된 도구 배지 표시
  - 도구 선택 이유 expander
  - 처리 과정 expander
  - 답변 복사 버튼
- **용어 추출/저장**: `extract_and_save_terms()`
  - 답변에서 AI 용어 자동 추출
  - PostgreSQL glossary 테이블에 저장
- **답변 품질 평가**: `AnswerEvaluator`
  - LLM-as-a-Judge 방식
  - 4가지 평가 항목 (정확도, 관련성, 난이도 적합성, 출처 명시)
  - 평가 결과 DB 저장 및 UI 표시

### 9. 계속 질문
- **평가 결과 표시**: expander로 평가 점수 표시
- **입력창 복귀**: Streamlit은 자동으로 입력창 활성화
- **세션 유지**: 브라우저 탭을 닫지 않는 한 세션 유지
  - 채팅 히스토리 보존
  - Agent 그래프 캐시 유지
  - ExperimentManager 상태 유지

> **핵심 포인트**:
> - **main.py는 Streamlit 서버 실행기**: 실제 로직은 ui/app.py에 있음
> - **Agent 캐싱**: `@st.cache_resource`로 Agent를 날짜별로 캐시하여 성능 최적화
> - **ExperimentManager**: 모든 실행 과정을 로그로 기록하여 실험 추적 가능
> - **LangGraph 기반 Agent**: StateGraph로 구성된 조건부 라우팅 시스템
> - **Fallback Chain**: 도구 실패 시 자동으로 대체 도구 실행
> - **실시간 품질 평가**: LLM-as-a-Judge 방식으로 답변 품질 자동 평가
