# 00. 프로젝트 구조

## 문서 정보
- **작성일**: 2025-11-04
- **작성자**: 최현화[팀장]
- **최종 수정일**: 2025-11-04

---

## 개요

본 문서는 논문 리뷰 챗봇 프로젝트의 **전체 디렉토리 구조와 파일 조직**을 설명합니다. 프로젝트는 **모듈화 원칙**에 따라 기능별로 분리된 디렉토리 구조를 가지고 있으며, 각 디렉토리는 명확한 책임을 가집니다.

### 핵심 설계 원칙
1. **관심사의 분리 (Separation of Concerns)**: 소스 코드, 스크립트, 데이터, 문서를 명확히 구분
2. **모듈화 (Modularity)**: 기능별로 독립된 모듈 구조
3. **계층화 (Layering)**: UI, 비즈니스 로직, 데이터 계층 분리
4. **확장성 (Scalability)**: 새로운 기능 추가가 용이한 구조

---

## 프로젝트 구조도

```mermaid
graph TB
    subgraph Root["🔸 프로젝트 루트"]
        direction LR
        Main[main.py<br/>진입점] --> Config[.env<br/>환경 변수]
        Config --> Req[requirements.txt<br/>의존성]
    end

    subgraph Source["🔹 소스 코드 (src/)"]
        direction LR
        Agent[agent/<br/>AI Agent<br/>Fallback Chain] --> Tools[tools/<br/>도구 시스템]
        Tools --> RAG[rag/<br/>RAG 검색]
        RAG --> LLM[llm/<br/>LLM 클라이언트]
        LLM --> DB[database/<br/>DB 연결]
        DB --> Utils[utils/<br/>유틸리티]
        Utils --> Eval[evaluation/<br/>성능 평가]
        Eval --> Mem[memory/<br/>대화 메모리]
    end

    subgraph UI["🔺 사용자 인터페이스 (ui/)"]
        direction LR
        App[app.py<br/>메인 UI] --> Comp[components/<br/>UI 컴포넌트]
        Comp --> Pages[pages/<br/>추가 페이지]
    end

    subgraph Scripts["🔶 스크립트 (scripts/)"]
        direction LR
        Data[data/<br/>데이터 처리] --> System[system/<br/>시스템 관리]
        System --> Tests[tests/<br/>테스트]
    end

    subgraph Docs["🔷 문서 (docs/)"]
        direction LR
        Mod[modularization/<br/>모듈 문서] --> PRD2[PRD/<br/>요구사항]
        PRD2 --> Roles[roles/<br/>담당역할]
        Roles --> Issues[issues/<br/>이슈 트래킹]
    end

    Root --> Source
    Source --> UI
    UI --> Scripts
    Scripts --> Docs

    %% Subgraph 스타일
    style Root fill:#e1f5ff,stroke:#01579b,stroke-width:3px,color:#000
    style Source fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style UI fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Scripts fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000
    style Docs fill:#fce4ec,stroke:#880e4f,stroke-width:3px,color:#000

    %% 노드 스타일 (프로젝트 루트 - 파랑 계열)
    style Main fill:#90caf9,stroke:#1976d2,color:#000
    style Config fill:#81d4fa,stroke:#0288d1,color:#000
    style Req fill:#64b5f6,stroke:#1976d2,color:#000

    %% 노드 스타일 (소스 코드 - 보라 계열)
    style Agent fill:#ce93d8,stroke:#7b1fa2,color:#000
    style Tools fill:#ba68c8,stroke:#7b1fa2,color:#fff
    style RAG fill:#ab47bc,stroke:#4a148c,color:#fff
    style LLM fill:#9c27b0,stroke:#4a148c,color:#fff
    style DB fill:#8e24aa,stroke:#4a148c,color:#fff
    style Utils fill:#7b1fa2,stroke:#4a148c,color:#fff
    style Eval fill:#6a1b9a,stroke:#4a148c,color:#fff
    style Mem fill:#4a148c,stroke:#4a148c,color:#fff

    %% 노드 스타일 (UI - 녹색 계열)
    style App fill:#a5d6a7,stroke:#388e3c,color:#000
    style Comp fill:#81c784,stroke:#2e7d32,color:#000
    style Pages fill:#66bb6a,stroke:#2e7d32,color:#fff

    %% 노드 스타일 (스크립트 - 주황 계열)
    style Data fill:#ffcc80,stroke:#f57c00,color:#000
    style System fill:#ffb74d,stroke:#f57c00,color:#000
    style Tests fill:#ffa726,stroke:#ef6c00,color:#000

    %% 노드 스타일 (문서 - 분홍 계열)
    style Mod fill:#f48fb1,stroke:#880e4f,color:#000
    style PRD2 fill:#f06292,stroke:#880e4f,color:#000
    style Roles fill:#ec407a,stroke:#880e4f,color:#fff
    style Issues fill:#e91e63,stroke:#880e4f,color:#fff
```

**구조도 설명:**
- **프로젝트 루트**: 진입점과 설정 파일
- **소스 코드 (src/)**: 핵심 비즈니스 로직 (Agent, Tools, RAG, LLM, Database, Evaluation 등)
- **사용자 인터페이스 (ui/)**: Streamlit 기반 웹 UI
- **스크립트 (scripts/)**: 데이터 처리, 시스템 관리, 테스트 스크립트
- **문서 (docs/)**: 설계 문서, PRD, 역할 문서, 이슈 트래킹

---

## 전체 아키텍처 (단순 흐름도)

```mermaid
graph TB
    subgraph MainFlow["📋 main.py 실행 워크플로우"]
        direction TB

        subgraph Init["🔸 초기화"]
            direction LR
            Start([▶️ python main.py]) --> LoadEnv[.env 파일<br/>환경변수 로드]
            LoadEnv --> CheckPath[ui/app.py<br/>경로 확인]
        end

        subgraph Streamlit["🔹 Streamlit 서버 시작"]
            direction LR
            RunStreamlit[Streamlit 실행<br/>ui/app.py] --> SetConfig[서버 설정<br/>port: 8501]
            SetConfig --> OpenBrowser[브라우저<br/>자동 열기]
        end

        subgraph UIInit["🔺 UI 초기화 (ui/app.py)"]
            direction LR
            PageConfig[페이지 설정<br/>제목/아이콘] --> ValidateEnv{환경변수<br/>검증}
            ValidateEnv -->|API 키 없음| ErrorStop[❌ 앱 중지]
            ValidateEnv -->|정상| InitAgent[Agent 초기화<br/>캐싱]
        end

        subgraph AgentSetup["🔶 Agent & 실험관리자 생성"]
            direction LR
            CreateExpMgr[실험관리자<br/>생성] --> CreateGraph[Agent 그래프<br/>구성]
            CreateGraph --> AddNodes[노드 추가<br/>라우터 + 7개 도구]
            AddNodes --> AddEdges[엣지 추가<br/>조건부 라우팅]
            AddEdges --> Compile[그래프<br/>컴파일]
        end

        subgraph UIRender["💡 UI 렌더링"]
            direction LR
            RenderSidebar[사이드바 렌더링<br/>난이도 선택] --> InitChat[채팅 세션<br/>초기화]
            InitChat --> ShowHistory[채팅 히스토리<br/>표시]
            ShowHistory --> WaitInput[사용자 입력<br/>대기]
        end

        subgraph UserInput["🔷 사용자 입력 처리"]
            direction LR
            InputReceived[질문 입력<br/>수신] --> SelectDiff[난이도 선택<br/>초심자/전문가]
            SelectDiff --> AddUserMsg[사용자 메시지<br/>추가]
            AddUserMsg --> InvokeAgent[Agent 그래프<br/>실행]
        end

        subgraph AgentExec["✨ Agent 실행 (LangGraph)"]
            direction LR
            RouterNode[라우터<br/>도구 선택] --> SelectTool{도구<br/>선택}
            SelectTool -->|일반| GeneralNode[일반 답변]
            SelectTool -->|RAG 논문| SearchNode[RAG 논문 검색]
            SelectTool -->|Web 논문| WebNode[Web 논문 검색]
            SelectTool -->|RAG 용어| GlossaryNode[RAG 용어집 검색]
            SelectTool -->|요약| SummarizeNode[논문 요약]
            SelectTool -->|통계| Text2SQLNode[Text2SQL 통계]
            SelectTool -->|저장| SaveNode[파일 저장]
        end

        subgraph DBConnect["💾 DB 연결"]
            direction LR
            PGVectorDB[(PGVector<br/>벡터 DB)]
            PostgresDB[(PostgreSQL<br/>RDBMS)]
            SearchNode -.-> PGVectorDB
            GlossaryNode -.-> PostgresDB
            Text2SQLNode -.-> PostgresDB
        end

        subgraph FallbackChain["🔄 도구 자동 전환 (Fallback)"]
            direction LR
            GlossaryFail[RAG 용어집 검색<br/>실패] -.-> GlossaryToGeneral[일반 답변<br/>전환]
            SearchFail[RAG 논문 검색<br/>실패] -.-> SearchToWeb[Web 논문 검색<br/>전환]
            SearchToWeb -.-> WebToGeneral[일반 답변<br/>전환]
            Text2SQLFail[Text2SQL 통계<br/>실패] -.-> Text2SQLToGeneral[일반 답변<br/>전환]
        end

        subgraph Response["🔸 응답 처리"]
            direction LR
            GetResponse[응답<br/>수신] --> ApplyDiff[난이도 반영<br/>최종 답변]
            ApplyDiff --> DisplayAnswer[답변<br/>표시]
            DisplayAnswer --> ExtractTerms[용어<br/>추출/저장]
            ExtractTerms --> Evaluate[답변<br/>품질 평가]
        end

        subgraph Continue["🔹 계속 질문"]
            direction LR
            ShowEval[평가 결과<br/>표시] --> WaitNext{계속<br/>질문?}
            WaitNext -->|Yes| BackToInput[입력창<br/>복귀]
            WaitNext -->|No| SessionEnd[세션<br/>유지]
        end

        %% 시작/종료 노드
        Start([▶️ 시작<br/>사용자 실행]) --> Init
        SessionEnd --> End([✅ 종료<br/>브라우저 탭 닫기])

        %% 단계 간 연결
        Init --> Streamlit
        Streamlit --> UIInit
        UIInit --> AgentSetup
        AgentSetup --> UIRender
        UIRender --> UserInput
        UserInput --> AgentExec
        AgentExec --> Response
        Response --> Continue
        BackToInput --> UserInput
    end

    %% 메인 워크플로우 배경
    style MainFlow fill:#fffde7,stroke:#f9a825,stroke-width:4px,color:#000

    %% Subgraph 스타일
    style Init fill:#e0f7fa,stroke:#006064,stroke-width:3px,color:#000
    style Streamlit fill:#e3f2fd,stroke:#0d47a1,stroke-width:3px,color:#000
    style UIInit fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style AgentSetup fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000
    style UIRender fill:#ffebee,stroke:#c62828,stroke-width:3px,color:#000
    style UserInput fill:#fce4ec,stroke:#880e4f,stroke-width:3px,color:#000
    style AgentExec fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Response fill:#e1f5fe,stroke:#01579b,stroke-width:3px,color:#000
    style Continue fill:#f1f8e9,stroke:#33691e,stroke-width:3px,color:#000

    %% 노드 스타일 (시작/종료)
    style Start fill:#4db6ac,stroke:#00695c,stroke-width:3px,color:#000
    style End fill:#66bb6a,stroke:#2e7d32,stroke-width:3px,color:#000

    %% 노드 스타일 (초기화 - 청록)
    style LoadEnv fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000
    style CheckPath fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000

    %% 노드 스타일 (Streamlit - 파랑)
    style RunStreamlit fill:#90caf9,stroke:#1976d2,stroke-width:2px,color:#000
    style SetConfig fill:#64b5f6,stroke:#1976d2,stroke-width:2px,color:#000
    style OpenBrowser fill:#42a5f5,stroke:#1565c0,stroke-width:2px,color:#000

    %% 노드 스타일 (UI 초기화 - 보라)
    style PageConfig fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style ValidateEnv fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#000
    style ErrorStop fill:#ef5350,stroke:#c62828,stroke-width:2px,color:#000
    style InitAgent fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000

    %% 노드 스타일 (Agent 생성 - 주황)
    style CreateExpMgr fill:#ffcc80,stroke:#f57c00,stroke-width:2px,color:#000
    style CreateGraph fill:#ffb74d,stroke:#f57c00,stroke-width:2px,color:#000
    style AddNodes fill:#ffa726,stroke:#ef6c00,stroke-width:2px,color:#000
    style AddEdges fill:#ff9800,stroke:#e65100,stroke-width:2px,color:#000
    style Compile fill:#fb8c00,stroke:#e65100,stroke-width:2px,color:#000

    %% 노드 스타일 (UI 렌더링 - 빨강)
    style RenderSidebar fill:#ef9a9a,stroke:#c62828,stroke-width:2px,color:#000
    style InitChat fill:#e57373,stroke:#c62828,stroke-width:2px,color:#000
    style ShowHistory fill:#ef5350,stroke:#b71c1c,stroke-width:2px,color:#000
    style WaitInput fill:#f44336,stroke:#b71c1c,stroke-width:2px,color:#000

    %% 노드 스타일 (사용자 입력 - 핑크)
    style InputReceived fill:#f48fb1,stroke:#ad1457,stroke-width:2px,color:#000
    style SelectDiff fill:#f48fb1,stroke:#ad1457,stroke-width:2px,color:#000
    style AddUserMsg fill:#f06292,stroke:#ad1457,stroke-width:2px,color:#000
    style InvokeAgent fill:#ec407a,stroke:#880e4f,stroke-width:2px,color:#000

    %% 노드 스타일 (Agent 실행 - 녹색)
    style RouterNode fill:#81c784,stroke:#2e7d32,stroke-width:2px,color:#000
    style SelectTool fill:#66bb6a,stroke:#2e7d32,stroke-width:2px,color:#000
    style GeneralNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style SearchNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style WebNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style GlossaryNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style SummarizeNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style Text2SQLNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style SaveNode fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000

    %% Subgraph 스타일 (DB 연결 - 보라)
    style DBConnect fill:#ede7f6,stroke:#512da8,stroke-width:3px,color:#000

    %% 노드 스타일 (DB - 보라)
    style PGVectorDB fill:#b39ddb,stroke:#512da8,stroke-width:2px,color:#000
    style PostgresDB fill:#b39ddb,stroke:#512da8,stroke-width:2px,color:#000

    %% Subgraph 스타일 (Fallback - 주황)
    style FallbackChain fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000

    %% 노드 스타일 (Fallback - 주황)
    style GlossaryFail fill:#ffcc80,stroke:#f57c00,stroke-width:2px,color:#000
    style GlossaryToGeneral fill:#ffb74d,stroke:#f57c00,stroke-width:2px,color:#000
    style SearchFail fill:#ffcc80,stroke:#f57c00,stroke-width:2px,color:#000
    style SearchToWeb fill:#ffa726,stroke:#ef6c00,stroke-width:2px,color:#000
    style WebToGeneral fill:#ffb74d,stroke:#f57c00,stroke-width:2px,color:#000
    style Text2SQLFail fill:#ffcc80,stroke:#f57c00,stroke-width:2px,color:#000
    style Text2SQLToGeneral fill:#ffb74d,stroke:#f57c00,stroke-width:2px,color:#000

    %% 노드 스타일 (응답 처리 - 하늘색)
    style GetResponse fill:#81d4fa,stroke:#0288d1,stroke-width:2px,color:#000
    style ApplyDiff fill:#81d4fa,stroke:#0288d1,stroke-width:2px,color:#000
    style DisplayAnswer fill:#4fc3f7,stroke:#0277bd,stroke-width:2px,color:#000
    style ExtractTerms fill:#29b6f6,stroke:#01579b,stroke-width:2px,color:#000
    style Evaluate fill:#03a9f4,stroke:#01579b,stroke-width:2px,color:#000

    %% 노드 스타일 (계속 - 연두)
    style ShowEval fill:#aed581,stroke:#558b2f,stroke-width:2px,color:#000
    style WaitNext fill:#9ccc65,stroke:#558b2f,stroke-width:2px,color:#000
    style BackToInput fill:#8bc34a,stroke:#33691e,stroke-width:2px,color:#000
    style SessionEnd fill:#aed581,stroke:#558b2f,stroke-width:2px,color:#000

    %% 연결선 스타일 (Continue 내부 0~2)
    linkStyle 0 stroke:#558b2f,stroke-width:2px
    linkStyle 1 stroke:#558b2f,stroke-width:2px
    linkStyle 2 stroke:#558b2f,stroke-width:2px

    %% 연결선 스타일 (시작/종료 3~4)
    linkStyle 3 stroke:#00695c,stroke-width:3px
    linkStyle 4 stroke:#2e7d32,stroke-width:3px

    %% 연결선 스타일 (초기화 5~6)
    linkStyle 5 stroke:#006064,stroke-width:2px
    linkStyle 6 stroke:#006064,stroke-width:2px

    %% 연결선 스타일 (Streamlit 7~8)
    linkStyle 7 stroke:#1976d2,stroke-width:2px
    linkStyle 8 stroke:#1976d2,stroke-width:2px

    %% 연결선 스타일 (UI 초기화 9~11)
    linkStyle 9 stroke:#7b1fa2,stroke-width:2px
    linkStyle 10 stroke:#c62828,stroke-width:2px
    linkStyle 11 stroke:#7b1fa2,stroke-width:2px

    %% 연결선 스타일 (Agent 생성 12~15)
    linkStyle 12 stroke:#f57c00,stroke-width:2px
    linkStyle 13 stroke:#f57c00,stroke-width:2px
    linkStyle 14 stroke:#f57c00,stroke-width:2px
    linkStyle 15 stroke:#f57c00,stroke-width:2px

    %% 연결선 스타일 (UI 렌더링 16~18)
    linkStyle 16 stroke:#c62828,stroke-width:2px
    linkStyle 17 stroke:#c62828,stroke-width:2px
    linkStyle 18 stroke:#c62828,stroke-width:2px

    %% 연결선 스타일 (사용자 입력 19~21)
    linkStyle 19 stroke:#ad1457,stroke-width:2px
    linkStyle 20 stroke:#ad1457,stroke-width:2px
    linkStyle 21 stroke:#ad1457,stroke-width:2px

    %% 연결선 스타일 (Agent 실행 22~29)
    linkStyle 22 stroke:#2e7d32,stroke-width:2px
    linkStyle 23 stroke:#2e7d32,stroke-width:2px
    linkStyle 24 stroke:#2e7d32,stroke-width:2px
    linkStyle 25 stroke:#2e7d32,stroke-width:2px
    linkStyle 26 stroke:#2e7d32,stroke-width:2px
    linkStyle 27 stroke:#2e7d32,stroke-width:2px
    linkStyle 28 stroke:#2e7d32,stroke-width:2px
    linkStyle 29 stroke:#2e7d32,stroke-width:2px

    %% 연결선 스타일 (응답 처리 30~33)
    linkStyle 30 stroke:#0288d1,stroke-width:2px
    linkStyle 31 stroke:#0288d1,stroke-width:2px
    linkStyle 32 stroke:#0288d1,stroke-width:2px
    linkStyle 33 stroke:#0288d1,stroke-width:2px

    %% 단계 간 연결 (회색 34~42)
    linkStyle 34 stroke:#616161,stroke-width:3px
    linkStyle 35 stroke:#616161,stroke-width:3px
    linkStyle 36 stroke:#616161,stroke-width:3px
    linkStyle 37 stroke:#616161,stroke-width:3px
    linkStyle 38 stroke:#616161,stroke-width:3px
    linkStyle 39 stroke:#616161,stroke-width:3px
    linkStyle 40 stroke:#616161,stroke-width:3px
    linkStyle 41 stroke:#616161,stroke-width:3px
    linkStyle 42 stroke:#616161,stroke-width:3px
```

---

## Agent 실행 상세 (Fallback Chain & 다중요청)

```mermaid
graph TB
    subgraph AgentDetail["🤖 Agent 실행 내부 구조 (LangGraph)"]
        direction TB

        subgraph RouterPhase["🔸 라우팅 단계"]
            direction LR
            RouterStart[router_node<br/>시작] --> MultiCheck{다중요청<br/>패턴?}
            MultiCheck -->|Yes| MultiRoute[패턴 매칭<br/>multi_request_patterns.yaml]
            MultiCheck -->|No| SingleRoute[단일요청<br/>라우팅]
            MultiRoute --> SetPipeline[tool_pipeline<br/>설정]
            SingleRoute --> ClassifyQ[질문 유형<br/>분류]
            ClassifyQ --> SetChain[Fallback Chain<br/>설정]
        end

        subgraph ValidationPhase["🔹 검증 단계"]
            direction LR
            Validate{Router<br/>검증?} -->|활성화| ValidateNode[validate_tool_choice_node<br/>도구 선택 검증]
            Validate -->|비활성화| SkipValidate[검증 스킵]
            ValidateNode -->|실패| RetryRouter[router 재실행<br/>max: 2회]
            ValidateNode -->|통과| ProceedTool[도구 실행<br/>진행]
            SkipValidate --> ProceedTool
        end

        subgraph ToolExecPhase["🔺 도구 실행 단계"]
            direction LR
            ToolNode[선택된 도구<br/>실행] --> WrapCheck{tool_wrapper<br/>적용?}
            WrapCheck -->|Yes| AutoStatus[tool_status<br/>자동 설정]
            WrapCheck -->|No| ManualStatus[기존 방식]
            AutoStatus --> ToolResult[도구 결과<br/>반환]
            ManualStatus --> ToolResult
        end

        subgraph FailurePhase["🔶 실패 감지 단계"]
            direction LR
            CheckStatus{tool_status<br/>확인} -->|success| NextCheck{다음<br/>도구?}
            CheckStatus -->|failed| FailDetect[FailureDetector<br/>실패 원인 분석]
            FailDetect --> CountRetry{재시도<br/>횟수?}
            CountRetry -->|< max_retries| FallbackRoute[fallback_router_node<br/>다음 도구 선택]
            CountRetry -->|≥ max_retries| FinalFallback[final_fallback_node<br/>general 강제 실행]
            NextCheck -->|Yes| PipelineRouter[pipeline_router<br/>다음 도구]
            NextCheck -->|No| Success([✅ 성공<br/>최종 답변])
        end

        subgraph FallbackPhase["💡 Fallback 체인 단계"]
            direction LR
            FallbackStart[Fallback Chain<br/>조회] --> GetNext{다음<br/>도구?}
            GetNext -->|있음| NextTool[다음 우선순위<br/>도구 선택]
            GetNext -->|없음| ForceGeneral[general 도구<br/>강제 실행]
            NextTool --> UpdateChain[failed_tools<br/>업데이트]
            ForceGeneral --> UpdateChain
            UpdateChain --> RetryTool[도구 재실행]
        end

        subgraph MultiTurnPhase["🔷 멀티턴 대화"]
            direction LR
            History[messages<br/>히스토리] --> Context[이전 대화<br/>문맥 전달]
            Context --> ContextRouter[라우터에<br/>문맥 반영]
            ContextRouter --> ContextTool[도구에<br/>문맥 전달]
        end

        %% 단계 간 연결
        RouterPhase --> ValidationPhase
        ValidationPhase --> ToolExecPhase
        ToolExecPhase --> FailurePhase
        FailurePhase -.Fallback.-> FallbackPhase
        FallbackPhase -.Retry.-> ToolExecPhase
        MultiTurnPhase -.Context.-> RouterPhase
    end

    %% 메인 배경
    style AgentDetail fill:#fffde7,stroke:#f9a825,stroke-width:4px,color:#000

    %% Subgraph 스타일
    style RouterPhase fill:#e0f7fa,stroke:#006064,stroke-width:3px,color:#000
    style ValidationPhase fill:#e3f2fd,stroke:#0d47a1,stroke-width:3px,color:#000
    style ToolExecPhase fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style FailurePhase fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000
    style FallbackPhase fill:#ffebee,stroke:#c62828,stroke-width:3px,color:#000
    style MultiTurnPhase fill:#fce4ec,stroke:#880e4f,stroke-width:3px,color:#000

    %% 노드 스타일 (라우팅 - 청록)
    style RouterStart fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000
    style MultiCheck fill:#26c6da,stroke:#006064,stroke-width:2px,color:#000
    style MultiRoute fill:#00bcd4,stroke:#006064,stroke-width:2px,color:#000
    style SingleRoute fill:#00bcd4,stroke:#006064,stroke-width:2px,color:#000
    style SetPipeline fill:#00acc1,stroke:#00695c,stroke-width:2px,color:#000
    style ClassifyQ fill:#00acc1,stroke:#00695c,stroke-width:2px,color:#000
    style SetChain fill:#0097a7,stroke:#00695c,stroke-width:2px,color:#000

    %% 노드 스타일 (검증 - 파랑)
    style Validate fill:#90caf9,stroke:#1976d2,stroke-width:2px,color:#000
    style ValidateNode fill:#64b5f6,stroke:#1976d2,stroke-width:2px,color:#000
    style SkipValidate fill:#64b5f6,stroke:#1976d2,stroke-width:2px,color:#000
    style RetryRouter fill:#42a5f5,stroke:#1565c0,stroke-width:2px,color:#000
    style ProceedTool fill:#2196f3,stroke:#1565c0,stroke-width:2px,color:#000

    %% 노드 스타일 (도구 실행 - 보라)
    style ToolNode fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#000
    style WrapCheck fill:#ba68c8,stroke:#7b1fa2,stroke-width:2px,color:#000
    style AutoStatus fill:#ab47bc,stroke:#7b1fa2,stroke-width:2px,color:#000
    style ManualStatus fill:#ab47bc,stroke:#7b1fa2,stroke-width:2px,color:#000
    style ToolResult fill:#9c27b0,stroke:#4a148c,stroke-width:2px,color:#000

    %% 노드 스타일 (실패 감지 - 주황)
    style CheckStatus fill:#ffcc80,stroke:#f57c00,stroke-width:2px,color:#000
    style NextCheck fill:#ffb74d,stroke:#f57c00,stroke-width:2px,color:#000
    style FailDetect fill:#ffa726,stroke:#ef6c00,stroke-width:2px,color:#000
    style CountRetry fill:#ff9800,stroke:#ef6c00,stroke-width:2px,color:#000
    style FallbackRoute fill:#fb8c00,stroke:#e65100,stroke-width:2px,color:#000
    style FinalFallback fill:#f57c00,stroke:#e65100,stroke-width:2px,color:#000
    style PipelineRouter fill:#ef6c00,stroke:#e65100,stroke-width:2px,color:#000
    style Success fill:#66bb6a,stroke:#2e7d32,stroke-width:3px,color:#000

    %% 노드 스타일 (Fallback - 빨강)
    style FallbackStart fill:#ef9a9a,stroke:#c62828,stroke-width:2px,color:#000
    style GetNext fill:#e57373,stroke:#c62828,stroke-width:2px,color:#000
    style NextTool fill:#ef5350,stroke:#b71c1c,stroke-width:2px,color:#000
    style ForceGeneral fill:#f44336,stroke:#b71c1c,stroke-width:2px,color:#000
    style UpdateChain fill:#e53935,stroke:#b71c1c,stroke-width:2px,color:#000
    style RetryTool fill:#d32f2f,stroke:#b71c1c,stroke-width:2px,color:#000

    %% 노드 스타일 (멀티턴 - 핑크)
    style History fill:#f48fb1,stroke:#ad1457,stroke-width:2px,color:#000
    style Context fill:#f06292,stroke:#ad1457,stroke-width:2px,color:#000
    style ContextRouter fill:#ec407a,stroke:#880e4f,stroke-width:2px,color:#000
    style ContextTool fill:#e91e63,stroke:#880e4f,stroke-width:2px,color:#000

    %% 연결선 스타일 (기본)
    linkStyle default stroke:#616161,stroke-width:2px
```

**Agent 내부 실행 흐름 설명:**

### 1. 라우팅 단계 (graph.py:252→router_node)
- **router_node 시작**: 사용자 질문 수신
- **다중요청 패턴 매칭**: `configs/multi_request_patterns.yaml` 확인
  - **패턴 매칭 성공**: `tool_pipeline` 배열 설정 (예: `[search_paper, summarize]`)
  - **패턴 매칭 실패**: 단일요청 라우팅
- **질문 유형 분류**: `QuestionClassifier` 7가지 유형 분류
  - `general`, `glossary`, `rag_search`, `web_search`, `summarize`, `text2sql`, `save_file`
- **Fallback Chain 설정**: 질문 유형별 우선순위 체인 (`configs/fallback_chains.yaml`)
  - 예: `glossary → general`
  - 예: `search_paper → web_search → general`

### 2. 검증 단계 (graph.py:264→validator)
- **Router 검증 활성화 여부**: `validation_enabled` 플래그 확인
- **validate_tool_choice_node**: 도구 선택의 타당성 검증
  - LLM으로 질문과 선택된 도구의 적합성 평가
  - 검증 실패 시 router 재실행 (최대 2회)
- **검증 비활성화**: 바로 도구 실행 진행

### 3. 도구 실행 단계 (graph.py:253-259→tool_nodes)
- **tool_wrapper 적용 여부**: Fallback Chain 활성화 시 자동 적용
- **tool_status 자동 설정**:
  - 성공: `tool_status = "success"`
  - 실패: `tool_status = "failed"` + 실패 원인 기록
- **도구 결과 반환**: `tool_result`, `final_answer` 업데이트

### 4. 실패 감지 단계 (graph.py:291→check_pipeline_or_fallback)
- **tool_status 확인**: `success` 또는 `failed`
- **성공 시**:
  - **다음 도구 확인**: `tool_pipeline` 존재 여부
  - **pipeline_router**: 다음 도구 실행 (다중요청)
  - **없으면 종료**: 최종 답변 반환
- **실패 시**:
  - **FailureDetector**: 실패 원인 분석 (`agent/failure_detector.py`)
  - **재시도 횟수 확인**: `retry_count < max_retries` (기본 3회)
  - **fallback_router_node**: 다음 우선순위 도구 선택
  - **최대 재시도 초과**: `final_fallback_node` → general 강제 실행

### 5. Fallback 체인 단계 (graph.py:247→fallback_router)
- **Fallback Chain 조회**: `state["fallback_chain"]`에서 우선순위 리스트 조회
- **다음 도구 선택**:
  - 실패한 도구를 `failed_tools`에 추가
  - Fallback Chain에서 다음 도구 선택
  - 예: `glossary` 실패 → `general`
  - 예: `search_paper` 실패 → `web_search` → `general`
- **도구 재실행**: 선택된 도구로 다시 실행
- **Fallback Chain 소진**: `final_fallback_node` → general 강제 실행

### 6. 멀티턴 대화 (ui/components/chat_interface.py:71→handle_agent_response)
- **messages 히스토리**: `st.session_state.messages` 배열
  - 이전 대화 내용 저장 (`role: user/assistant`, `content`)
- **문맥 전달**: `agent_executor.invoke()`에 `messages` 전달
  ```python
  state = {
      "question": user_input,
      "difficulty": difficulty,
      "messages": st.session_state.messages  # 이전 대화 포함
  }
  ```
- **라우터에 문맥 반영**: router_node에서 `messages` 참조
- **도구에 문맥 전달**: 각 도구 노드에서 `messages` 활용
  - 예: general_answer_node에서 이전 대화 맥락으로 답변 생성

### 7. 다중요청 처리 (graph.py:324→pipeline_router)
- **tool_pipeline 순차 실행**: `[search_paper, summarize]`
  - 1단계: `search_paper` 실행 → 논문 검색
  - 2단계: `summarize` 실행 → 검색된 논문 요약
- **pipeline_index 업데이트**: 매 도구 실행 후 증가
- **도구 실패 시**: Pipeline 중단 → Fallback Chain 진입
- **검색 도구 스킵 로직** (graph.py:333-353):
  - `search_paper` 성공 시 → `web_search`, `general` 스킵 → `summarize`로 이동
  - `web_search` 성공 시 → `general` 스킵 → `summarize`로 이동

> **핵심 포인트**:
> - **Fallback Chain**: 도구 실패 시 자동으로 대체 도구 실행 (최대 3회 재시도)
> - **다중요청 처리**: 패턴 매칭으로 `tool_pipeline` 설정 → 순차 실행
> - **멀티턴 대화**: `messages` 히스토리를 Agent에 전달하여 문맥 유지
> - **도구 자동 전환**: 검색 성공 시 중복 검색 도구 스킵 (성능 최적화)
> - **Router 검증**: LLM으로 도구 선택의 타당성 검증 (선택적)
> - **tool_wrapper**: 도구 실행 전후로 `tool_status` 자동 설정
>
> 상세한 문서:
> - **Fallback Chain**: **[06-1_도구_자동전환_기능.md](./06-1_도구_자동전환_기능.md)**
> - **다중요청 패턴**: **[docs/architecture/multiple_request/](../architecture/multiple_request/)**
> - **질문 분류**: **[03_AI_Agent_시스템.md](./03_AI_Agent_시스템.md)** - QuestionClassifier 섹션
> - **멀티턴 대화**: **[05_대화_메모리_시스템.md](./05_대화_메모리_시스템.md)**

---

**main.py 실행 흐름 설명:**

### 1. 초기화 단계 (main.py)
- **python main.py 실행**: 사용자가 CLI에서 프로그램 시작
- **.env 파일 로드**: `load_dotenv()`로 환경변수 읽기
  - OPENAI_API_KEY
  - POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_HOST, POSTGRES_DB
  - SOLAR_API_KEY
  - TAVILY_API_KEY
- **ui/app.py 경로 확인**: Streamlit UI 파일 존재 여부 검증

### 2. Streamlit 서버 시작
- **Streamlit 실행**: `ui/app.py` 파일을 웹 서버로 실행
- **서버 설정**:
  - 포트: 8501
  - 헤드리스 모드: false (브라우저 자동 열기)
  - 테마: light
- **브라우저 자동 열기**: http://localhost:8501

### 3. UI 초기화 (ui/app.py)
- **페이지 설정**: `st.set_page_config()`
  - 제목: "논문 리뷰 챗봇"
  - 파비콘: 📚
  - 레이아웃: wide
- **환경변수 검증**:
  - OPENAI_API_KEY 필수 → 없으면 앱 중지
  - PostgreSQL 설정 확인 → 없으면 경고 표시
- **Agent 초기화 (캐싱)**: 캐시 데코레이터로 Agent 재사용
  - 날짜별로 캐시 키 생성 (날짜 변경 시 재초기화)

### 4. Agent & 실험관리자 생성
- **실험관리자 생성**: 실험 로그 관리 시스템 초기화
  - 실험 폴더 생성: `experiments/YYYYMMDD/HHMMSS/`
  - 로거 설정: 각 도구별 로그 파일
- **Agent 그래프 구성**: LangGraph 기반 AI Agent 그래프 생성
- **노드 추가**: 라우터 + 7개 도구 노드
  - **일반 답변**: LLM 직접 호출 (DB 연결 없음)
  - **RAG 논문 검색**: PGVector (벡터 DB)
  - **Web 논문 검색**: Tavily API
  - **RAG 용어집 검색**: PostgreSQL glossary 테이블
  - **논문 요약**: 파일 읽기 (PDF 분석)
  - **Text2SQL 통계**: PostgreSQL statistics 테이블
  - **파일 저장**: 로컬 파일 시스템
- **엣지 추가**: 조건부 라우팅 설정
- **그래프 컴파일**: 그래프 구조 완성

### 5. UI 렌더링
- **사이드바 렌더링**:
  - 난이도 선택 (초심자/전문가)
  - 새 채팅 시작 버튼
  - 채팅 히스토리 목록
- **채팅 세션 초기화**:
  - 첫 실행 시 자동으로 새 채팅 생성
- **채팅 히스토리 표시**:
  - 현재 채팅의 모든 메시지 렌더링
- **사용자 입력 대기**: 채팅 입력창 표시

### 6. 사용자 입력 처리
- **질문 입력 수신**: 사용자가 채팅 입력창에 질문 입력
- **난이도 선택**: 사이드바에서 초심자/전문가 모드 선택
  - **초심자 모드**: 쉬운 설명, 기본 개념 위주
  - **전문가 모드**: 전문 용어, 심화 내용 포함
- **사용자 메시지 추가**: 현재 채팅에 메시지 추가
  - 세션 상태에 메시지 저장
  - UI에 사용자 채팅 버블 표시
- **Agent 그래프 실행**: Agent 그래프를 실행하여 답변 생성
  - 입력: 질문, 난이도, 이전 대화 히스토리
  - 콜백: Streamlit 실시간 출력 핸들러

### 7. Agent 실행 (LangGraph)

#### 7.1 라우터 (도구 선택)
- **질문 분석 및 라우팅**:
  - 다중 요청 패턴 매칭 (`multi_request_patterns.yaml`)
  - 질문 유형 분류 (QuestionClassifier)
  - LLM으로 도구 선택
  - 키워드 매칭 Fallback
  - **난이도 정보 전달**: 초심자/전문가 모드를 각 도구에 전달

#### 7.2 도구 선택 및 실행
- **일반 답변**:
  - DB 연결: 없음
  - LLM 직접 호출
  - 난이도별 프롬프트: 초심자용/전문가용 구분

- **RAG 논문 검색**:
  - **DB 연결: PGVector (벡터 DB)**
  - PostgreSQL + PGVector 확장
  - 벡터 유사도 검색으로 관련 논문 검색

- **Web 논문 검색**:
  - 외부 API: Tavily Search API
  - 실시간 최신 논문 정보 검색

- **RAG 용어집 검색**:
  - **DB 연결: PostgreSQL `glossary` 테이블**
  - 난이도별 설명: `easy_explanation`, `hard_explanation` 컬럼

- **논문 요약**:
  - 파일 시스템: PDF 파일 직접 읽기
  - 난이도별 요약: 초심자용 간단 요약, 전문가용 상세 요약

- **Text2SQL 통계**:
  - **DB 연결: PostgreSQL `statistics` 테이블**
  - 자연어 → SQL 쿼리 자동 변환
  - 쿼리 실행 및 결과 반환

- **파일 저장**:
  - 파일 시스템: 로컬 디렉토리에 저장

#### 7.3 도구 자동 전환 (Fallback Chain)
- **RAG 용어집 검색 실패** → **일반 답변으로 전환**
- **RAG 논문 검색 실패** → **Web 논문 검색으로 전환** → **일반 답변으로 전환**
- **Text2SQL 통계 실패** → **일반 답변으로 전환**

#### 7.4 DB 연결 구조
- **PGVector (벡터 DB)**: RAG 논문 검색
- **PostgreSQL (RDBMS)**:
  - `glossary` 테이블: RAG 용어집 검색
  - `statistics` 테이블: Text2SQL 통계

### 8. 응답 처리
- **응답 수신**: Agent 그래프 실행 완료
  - final_answer: 최종 답변
  - tool_choice: 사용된 도구
  - routing_reason: 도구 선택 이유
  - tool_timeline: 도구 실행 과정
- **난이도 반영 (최종 답변)**: 선택된 난이도에 맞게 답변 후처리
  - **초심자 모드**: 전문 용어에 자동으로 간단 설명 추가
  - **전문가 모드**: 심화 내용 및 참고 문헌 포함
- **답변 표시**: 챗봇 메시지로 표시
  - 답변 내용 렌더링
  - 사용된 도구 배지 표시
  - 도구 선택 이유 펼침 메뉴
  - 처리 과정 펼침 메뉴
  - 답변 복사 버튼
- **용어 추출/저장**: 답변에서 AI 용어 추출 및 저장
  - 답변에서 AI 용어 자동 추출
  - PostgreSQL glossary 테이블에 저장
- **답변 품질 평가**: LLM-as-a-Judge 방식으로 평가
  - 4가지 평가 항목 (정확도, 관련성, **난이도 적합성**, 출처 명시)
  - 평가 결과 DB 저장 및 UI 표시

### 9. 계속 질문
- **평가 결과 표시**: 펼침 메뉴로 평가 점수 표시
- **입력창 복귀**: 자동으로 입력창 활성화
- **세션 유지**: 브라우저 탭을 닫지 않는 한 세션 유지
  - 채팅 히스토리 보존
  - Agent 그래프 캐시 유지
  - 실험관리자 상태 유지

> **핵심 포인트**:
> - **main.py는 Streamlit 서버 실행기**: 실제 로직은 ui/app.py에 있음
> - **Agent 캐싱**: Agent를 날짜별로 캐시하여 성능 최적화
> - **실험관리자**: 모든 실행 과정을 로그로 기록하여 실험 추적 가능
> - **LangGraph 기반 Agent**: 조건부 라우팅 시스템
> - **7개 도구 시스템**:
>   1. **일반 답변**: LLM 직접 호출
>   2. **RAG 논문 검색**: PGVector (벡터 DB)
>   3. **Web 논문 검색**: Tavily API
>   4. **RAG 용어집 검색**: PostgreSQL glossary 테이블
>   5. **논문 요약**: PDF 파일 읽기
>   6. **Text2SQL 통계**: PostgreSQL statistics 테이블
>   7. **파일 저장**: 로컬 파일 시스템
> - **DB 연결 구조**:
>   - **PGVector**: RAG 논문 검색 (벡터 유사도)
>   - **PostgreSQL**: RAG 용어집 검색, Text2SQL 통계
> - **도구 자동 전환 (Fallback Chain)**:
>   - RAG 용어집 검색 실패 → 일반 답변
>   - RAG 논문 검색 실패 → Web 논문 검색 → 일반 답변
>   - Text2SQL 통계 실패 → 일반 답변
> - **난이도별 답변**: 초심자/전문가 모드에 따라 프롬프트 및 답변 스타일 변경
> - **실시간 품질 평가**: LLM-as-a-Judge 방식으로 답변 품질 자동 평가 (난이도 적합성 포함)
> - **용어 자동 추출**: 답변에서 AI 용어를 자동으로 추출하여 DB에 저장
> - **멀티턴 대화**: 이전 대화 히스토리를 Agent에 전달하여 문맥 유지
>
> 상세한 각 단계별 문서:
> - Agent 시스템: **[03_AI_Agent_시스템.md](./03_AI_Agent_시스템.md)**
> - Fallback Chain: **[06-1_도구_자동전환_기능.md](./06-1_도구_자동전환_기능.md)**
> - ExperimentManager: **[02_실험_관리_시스템.md](./02_실험_관리_시스템.md)**
> - Streamlit UI: **[07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md)**
> - 성능 평가: **[10_성능평가시스템.md](./10_성능평가시스템.md)**

---

## 전체 디렉토리 구조

```
langchain-project/
│
├── 📄 main.py                          # 메인 진입점 (AI Agent 실행)
├── 📄 requirements.txt                 # Python 의존성 패키지
├── 📄 .env                             # 환경 변수 (API 키, DB 설정)
├── 📄 .env.example                     # 환경 변수 템플릿
├── 📄 .gitignore                       # Git 무시 파일
├── 📄 README.md                        # 프로젝트 소개
│
├── 📂 src/                             # 소스 코드
│   ├── 📂 agent/                       # AI Agent 시스템
│   │   ├── __init__.py
│   │   ├── state.py                    # Agent 상태 정의
│   │   ├── graph.py                    # LangGraph 그래프 구성
│   │   ├── nodes.py                    # Agent 노드 정의
│   │   ├── failure_detector.py         # 도구 실패 감지
│   │   ├── question_classifier.py      # 질문 유형 분류
│   │   ├── config_loader.py            # Fallback 설정 로더
│   │   └── tool_wrapper.py             # 도구 래퍼 (자동 상태 설정)
│   │
│   ├── 📂 tools/                       # AI Agent 도구
│   │   ├── __init__.py
│   │   ├── general_answer.py           # 일반 답변 도구
│   │   ├── search_paper.py             # 논문 검색 도구
│   │   ├── summarize.py                # 논문 요약 도구
│   │   ├── glossary.py                 # 용어 설명 도구
│   │   ├── web_search.py               # 웹 검색 도구
│   │   ├── text2sql.py                 # Text-to-SQL 도구
│   │   ├── arxiv_handler.py            # arXiv 논문 다운로드 도구
│   │   └── save_file.py                # 파일 저장 도구
│   │
│   ├── 📂 rag/                         # RAG 시스템
│   │   └── retriever.py                # RAG 검색 로직
│   │
│   ├── 📂 llm/                         # LLM 클라이언트
│   │   ├── __init__.py
│   │   └── client.py                   # OpenAI/Solar LLM 클라이언트
│   │
│   ├── 📂 database/                    # 데이터베이스 연결
│   │   ├── db.py                       # PostgreSQL 연결
│   │   ├── vector_store.py             # VectorDB 관리
│   │   └── embeddings.py               # Embeddings 생성
│   │
│   ├── 📂 memory/                      # 대화 메모리
│   │   ├── __init__.py
│   │   └── chat_history.py             # PostgreSQL 채팅 히스토리
│   │
│   ├── 📂 prompts/                     # 프롬프트 관리
│   │   ├── __init__.py
│   │   └── loader.py                   # 프롬프트 템플릿 로더
│   │
│   ├── 📂 data/                        # 데이터 로더
│   │   └── document_loader.py          # 문서 로딩
│   │
│   ├── 📂 utils/                       # 유틸리티
│   │   ├── __init__.py
│   │   ├── logger.py                   # 로깅 시스템
│   │   ├── experiment_manager.py       # 실험 관리 시스템
│   │   ├── config_loader.py            # 설정 파일 로더
│   │   └── glossary_extractor.py       # 용어 추출
│   │
│   ├── 📂 evaluation/                  # 성능 평가 시스템
│   │   ├── __init__.py
│   │   ├── evaluator.py                # LLM-as-a-Judge 평가
│   │   └── storage.py                  # 평가 결과 저장/조회
│   │
│   ├── 📂 papers/                      # 논문 도메인
│   │   ├── domain/
│   │   │   └── dto.py                  # 논문 DTO
│   │   └── infra/
│   │       └── arxiv_client.py         # arXiv API 클라이언트
│   │
│   ├── 📂 text2sql/                    # Text-to-SQL (예약됨)
│   │
│   └── __init__.py
│
├── 📂 ui/                              # Streamlit UI
│   ├── app.py                          # 메인 UI 진입점
│   │
│   ├── 📂 components/                  # UI 컴포넌트
│   │   ├── chat_interface.py           # 채팅 인터페이스
│   │   ├── chat_manager.py             # 채팅 세션 관리
│   │   ├── sidebar.py                  # 사이드바
│   │   ├── file_download.py            # 파일 다운로드
│   │   └── storage.py                  # 세션 스토리지
│   │
│   ├── 📂 pages/                       # 추가 페이지
│   │   └── 1_📚_Glossary.py            # 용어집 페이지
│   │
│   ├── 📂 test/                        # UI 테스트
│   │   └── paper_review_app.py         # 테스트 앱
│   │
│   ├── 📂 assets/                      # 정적 파일 (이미지 등)
│   │
│   └── __init__.py
│
├── 📂 scripts/                         # 스크립트
│   ├── evaluate_answers.py             # 답변 평가 스크립트
│   │
│   ├── 📂 data/                        # 데이터 처리 스크립트
│   │   ├── collect_arxiv_papers.py     # arXiv 논문 수집
│   │   ├── process_documents.py        # 문서 전처리
│   │   ├── load_embeddings.py          # 임베딩 생성 및 로드
│   │   ├── setup_database.py           # DB 초기 설정
│   │   ├── update_glossary_explanations.py  # 용어집 업데이트
│   │   ├── inspect_vector_db.py        # VectorDB 검사
│   │   └── run_full_pipeline.py        # 전체 파이프라인 실행
│   │
│   ├── 📂 system/                      # 시스템 관리 스크립트
│   │   ├── find_experiments.py         # 실험 로그 검색
│   │   └── aggregate_metrics.py        # 메트릭 집계
│   │
│   ├── 📂 tests/                       # 테스트 스크립트
│   │   ├── unit/                       # 단위 테스트
│   │   │   ├── test_db_connection.py   # DB 연결 테스트
│   │   │   ├── test_experiment_manager.py  # 실험 관리 테스트
│   │   │   ├── test_crud_operations.py # CRUD 테스트
│   │   │   ├── test_vector_store.py    # VectorDB 테스트
│   │   │   ├── test_rag.py             # RAG 테스트
│   │   │   ├── init_database.py        # DB 초기화 테스트
│   │   │   └── tools/                  # 도구 테스트
│   │   │       ├── test_arxiv_handler.py
│   │   │       └── test_summarize.py
│   │   │
│   │   └── integration/                # 통합 테스트
│   │       ├── test_integration.py     # 통합 테스트
│   │       └── test_data_pipeline.py   # 데이터 파이프라인 테스트
│   │
│   └── 📂 debug/                       # 디버깅 스크립트
│       ├── check_db.py                 # DB 상태 확인
│       ├── check_db2.py
│       ├── check_bert_chunks.py        # BERT 청크 확인
│       └── test_summarize_debug.py     # 요약 디버깅
│
├── 📂 data/                            # 데이터 디렉토리
│   ├── 📂 raw/                         # 원본 데이터
│   │   └── pdfs/                       # PDF 파일
│   │
│   ├── 📂 processed/                   # 전처리된 데이터
│   │
│   ├── 📂 rdbms/                       # RDBMS 데이터 (PostgreSQL)
│   │
│   └── 📂 vectordb/                    # VectorDB 데이터
│       └── papers_faiss/               # FAISS 인덱스
│
├── 📂 configs/                         # 설정 파일
│   └── collect/                        # 수집 설정
│
├── 📂 logs/                            # 로그 파일
│   └── experiments/                    # 실험 로그
│       └── YYYYMMDD_HHMMSS/            # 실험별 디렉토리
│
├── 📂 notebooks/                       # Jupyter 노트북
│   ├── base/                           # 기본 노트북
│   └── team/                           # 팀원 노트북
│
├── 📂 docs/                            # 문서
│   ├── 📂 modularization/              # 모듈화 문서
│   │   ├── 00_프로젝트_구조.md         # 프로젝트 구조 (본 문서)
│   │   ├── 01_개발환경_및_기술스택.md
│   │   ├── 01_로깅_시스템.md
│   │   ├── 02_실험_관리_시스템.md
│   │   ├── 03_AI_Agent_시스템.md
│   │   ├── 04_LLM_클라이언트.md
│   │   ├── 05_대화_메모리_시스템.md
│   │   ├── 05_프롬프트_엔지니어링.md
│   │   ├── 06_도구_시스템.md
│   │   ├── 06-1_도구_자동전환_기능.md  # Fallback Chain 메커니즘
│   │   ├── 07_Streamlit_UI_시스템.md
│   │   ├── 08_데이터베이스_시스템.md
│   │   ├── 09_RAG_시스템.md
│   │   └── 10_성능평가시스템.md
│   │
│   ├── 📂 PRD/                         # PRD (Product Requirements Document)
│   │   ├── 01_프로젝트_개요.md
│   │   ├── 02_유저_페르소나.md
│   │   ├── 03_핵심_기능_명세.md
│   │   ├── 04_AI_Agent_아키텍처.md
│   │   ├── 05_RAG_시스템_설계.md
│   │   ├── 06_데이터베이스_설계.md
│   │   ├── 07_UI_UX_설계.md
│   │   ├── 08_프롬프트_전략.md
│   │   ├── 09_평가_기준.md
│   │   └── 10_기술_요구사항.md
│   │
│   ├── 📂 roles/                       # 담당역할 문서
│   │   ├── 담당역할_01_이예포_로깅시스템.md
│   │   ├── 담당역할_02_윤정인_실험관리.md
│   │   ├── 담당역할_03_강도형_AI_Agent.md
│   │   ├── 담당역할_04_권지원_LLM_클라이언트.md
│   │   ├── 담당역할_05-1_신준엽_Text-to-SQL.md
│   │   └── 담당역할_05-2_최현화_성능평가시스템.md
│   │
│   ├── 📂 issues/                      # 이슈 트래킹 문서
│   │   ├── 05-1_Text-to-SQL_구현.md
│   │   ├── 05-2_성능평가시스템_구현.md
│   │   └── merge/                      # 병합 관련 이슈
│   │
│   ├── 📂 usage/                       # 사용 가이드
│   │   ├── team/                       # 팀원별 사용 가이드
│   │   └── tests/                      # 테스트 가이드
│   │
│   ├── 📂 architecture/                # 아키텍처 문서
│   │
│   ├── 📂 scenarios/                   # 시나리오 문서
│   │
│   ├── 📂 rules/                       # 코딩 규칙
│   │   ├── annotate_style.md           # 한글 주석 스타일 가이드
│   │   ├── mermaid_style.md            # Mermaid 다이어그램 스타일 가이드
│   │   └── commit_msg.md               # 커밋 메시지 가이드
│   │
│   ├── 📂 minutes/                     # 회의록
│   │   ├── 20251028/
│   │   ├── 20251029/
│   │   ├── 20251030/
│   │   ├── 20251031/
│   │   └── 20251103/
│   │
│   ├── 📂 experiments/                 # 실험 문서
│   │   └── 20251101/
│   │
│   └── 📂 errors/                      # 에러 기록
│
├── 📂 .streamlit/                      # Streamlit 설정
│   └── config.toml                     # Streamlit 설정 파일
│
└── 📂 .github/                         # GitHub 설정
    └── ISSUE_TEMPLATE/                 # 이슈 템플릿
```

---

## 디렉토리별 상세 설명

### 1. 프로젝트 루트

| 파일/디렉토리 | 설명 |
|-------------|------|
| **main.py** | 메인 진입점 - AI Agent 실행 |
| **requirements.txt** | Python 의존성 패키지 목록 |
| **.env** | 환경 변수 (API 키, DB 설정) - Git에 포함되지 않음 |
| **.env.example** | 환경 변수 템플릿 - Git에 포함됨 |
| **.gitignore** | Git 무시 파일 목록 |
| **README.md** | 프로젝트 소개 및 실행 가이드 |

---

### 2. src/ - 소스 코드

#### 2.1 agent/ - AI Agent 시스템

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **state.py** | Agent 상태 정의 (AgentState 클래스, Fallback 필드 포함) | [03_AI_Agent_시스템.md](./03_AI_Agent_시스템.md) |
| **graph.py** | LangGraph 그래프 구성 (create_graph, Fallback Chain 지원) | [03_AI_Agent_시스템.md](./03_AI_Agent_시스템.md), [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |
| **nodes.py** | Agent 노드 정의 (router, fallback_router, validator 등) | [03_AI_Agent_시스템.md](./03_AI_Agent_시스템.md), [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |
| **failure_detector.py** | 도구 실행 실패 감지 (FailureDetector 클래스) | [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |
| **question_classifier.py** | 질문 유형 분류 (QuestionClassifier, 7가지 유형) | [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |
| **config_loader.py** | Fallback Chain 설정 로더 (YAML 파싱) | [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |
| **tool_wrapper.py** | 도구 래퍼 (tool_status 자동 설정) | [09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md) |

#### 2.2 tools/ - AI Agent 도구

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **general_answer.py** | 일반 답변 도구 (쉬운/어려운 모드) | [06_도구_시스템.md](./06_도구_시스템.md) |
| **search_paper.py** | 논문 검색 도구 (RAG 검색) | [06_도구_시스템.md](./06_도구_시스템.md) |
| **summarize.py** | 논문 요약 도구 | [06_도구_시스템.md](./06_도구_시스템.md) |
| **glossary.py** | 용어 설명 도구 | [06_도구_시스템.md](./06_도구_시스템.md) |
| **web_search.py** | 웹 검색 도구 (Tavily API) | [06_도구_시스템.md](./06_도구_시스템.md) |
| **text2sql.py** | Text-to-SQL 도구 | [06_도구_시스템.md](./06_도구_시스템.md) |
| **arxiv_handler.py** | arXiv 논문 다운로드 도구 | [06_도구_시스템.md](./06_도구_시스템.md) |
| **save_file.py** | 파일 저장 도구 | [06_도구_시스템.md](./06_도구_시스템.md) |

#### 2.3 rag/ - RAG 시스템

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **retriever.py** | RAG 검색 로직 (PGVector + HyDE) | [09_RAG_시스템.md](./09_RAG_시스템.md) |

#### 2.4 llm/ - LLM 클라이언트

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **client.py** | OpenAI/Solar LLM 클라이언트 (Singleton) | [04_LLM_클라이언트.md](./04_LLM_클라이언트.md) |

#### 2.5 database/ - 데이터베이스 연결

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **db.py** | PostgreSQL 연결 및 CRUD 함수 | [08_데이터베이스_시스템.md](./08_데이터베이스_시스템.md) |
| **vector_store.py** | VectorDB 관리 (PGVector) | [08_데이터베이스_시스템.md](./08_데이터베이스_시스템.md) |
| **embeddings.py** | Embeddings 생성 (OpenAI) | [08_데이터베이스_시스템.md](./08_데이터베이스_시스템.md) |

#### 2.6 memory/ - 대화 메모리

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **chat_history.py** | PostgreSQL 채팅 히스토리 관리 | [05_대화_메모리_시스템.md](./05_대화_메모리_시스템.md) |

#### 2.7 prompts/ - 프롬프트 관리

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **loader.py** | 프롬프트 템플릿 로더 | [05_프롬프트_엔지니어링.md](./05_프롬프트_엔지니어링.md) |

#### 2.8 data/ - 데이터 로더

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **document_loader.py** | 문서 로딩 (PDF, 텍스트) | [09_RAG_시스템.md](./09_RAG_시스템.md) |

#### 2.9 utils/ - 유틸리티

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **logger.py** | 로깅 시스템 (Logger 클래스) | [01_로깅_시스템.md](./01_로깅_시스템.md) |
| **experiment_manager.py** | 실험 관리 시스템 (ExperimentManager) | [02_실험_관리_시스템.md](./02_실험_관리_시스템.md) |
| **config_loader.py** | 설정 파일 로더 (YAML) | - |
| **glossary_extractor.py** | 용어 추출 유틸리티 | - |

#### 2.10 evaluation/ - 성능 평가 시스템

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **evaluator.py** | LLM-as-a-Judge 평가 (AnswerEvaluator) | [10_성능평가시스템.md](./10_성능평가시스템.md) |
| **storage.py** | 평가 결과 저장/조회 (PostgreSQL) | [10_성능평가시스템.md](./10_성능평가시스템.md) |

#### 2.11 papers/ - 논문 도메인

| 파일 | 설명 | 문서 참조 |
|------|------|----------|
| **domain/dto.py** | 논문 DTO (Data Transfer Object) | - |
| **infra/arxiv_client.py** | arXiv API 클라이언트 | - |

---

### 3. ui/ - Streamlit UI

| 파일/디렉토리 | 설명 | 문서 참조 |
|-------------|------|----------|
| **app.py** | 메인 UI 진입점 (Streamlit 앱) | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **components/chat_interface.py** | 채팅 인터페이스 컴포넌트 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **components/chat_manager.py** | 채팅 세션 관리 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **components/sidebar.py** | 사이드바 컴포넌트 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **components/file_download.py** | 파일 다운로드 컴포넌트 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **components/storage.py** | 세션 스토리지 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **pages/1_📚_Glossary.py** | 용어집 페이지 | [07_Streamlit_UI_시스템.md](./07_Streamlit_UI_시스템.md) |
| **test/paper_review_app.py** | 테스트 앱 | - |
| **assets/** | 정적 파일 (이미지 등) | - |

---

### 4. scripts/ - 스크립트

#### 4.1 scripts/data/ - 데이터 처리 스크립트

| 파일 | 설명 | 실행 명령 |
|------|------|----------|
| **collect_arxiv_papers.py** | arXiv 논문 수집 | `python scripts/data/collect_arxiv_papers.py` |
| **process_documents.py** | 문서 전처리 (청킹) | `python scripts/data/process_documents.py` |
| **load_embeddings.py** | 임베딩 생성 및 VectorDB 로드 | `python scripts/data/load_embeddings.py` |
| **setup_database.py** | PostgreSQL DB 초기 설정 | `python scripts/data/setup_database.py` |
| **update_glossary_explanations.py** | 용어집 업데이트 | `python scripts/data/update_glossary_explanations.py` |
| **inspect_vector_db.py** | VectorDB 검사 | `python scripts/data/inspect_vector_db.py` |
| **run_full_pipeline.py** | 전체 데이터 파이프라인 실행 | `python scripts/data/run_full_pipeline.py` |

#### 4.2 scripts/system/ - 시스템 관리 스크립트

| 파일 | 설명 | 실행 명령 |
|------|------|----------|
| **find_experiments.py** | 실험 로그 검색 | `python scripts/system/find_experiments.py` |
| **aggregate_metrics.py** | 메트릭 집계 | `python scripts/system/aggregate_metrics.py` |

#### 4.3 scripts/tests/ - 테스트 스크립트

| 파일 | 설명 | 실행 명령 |
|------|------|----------|
| **unit/test_db_connection.py** | DB 연결 테스트 | `python scripts/tests/unit/test_db_connection.py` |
| **unit/test_experiment_manager.py** | 실험 관리 테스트 | `python scripts/tests/unit/test_experiment_manager.py` |
| **unit/test_crud_operations.py** | CRUD 테스트 | `python scripts/tests/unit/test_crud_operations.py` |
| **unit/test_vector_store.py** | VectorDB 테스트 | `python scripts/tests/unit/test_vector_store.py` |
| **unit/test_rag.py** | RAG 테스트 | `python scripts/tests/unit/test_rag.py` |
| **unit/init_database.py** | DB 초기화 테스트 | `python scripts/tests/unit/init_database.py` |
| **unit/tools/test_arxiv_handler.py** | arXiv 도구 테스트 | `python scripts/tests/unit/tools/test_arxiv_handler.py` |
| **unit/tools/test_summarize.py** | 요약 도구 테스트 | `python scripts/tests/unit/tools/test_summarize.py` |
| **integration/test_integration.py** | 통합 테스트 | `python scripts/tests/integration/test_integration.py` |
| **integration/test_data_pipeline.py** | 데이터 파이프라인 테스트 | `python scripts/tests/integration/test_data_pipeline.py` |

#### 4.4 scripts/debug/ - 디버깅 스크립트

| 파일 | 설명 | 실행 명령 |
|------|------|----------|
| **check_db.py** | DB 상태 확인 | `python scripts/debug/check_db.py` |
| **check_db2.py** | DB 상태 확인 (추가) | `python scripts/debug/check_db2.py` |
| **check_bert_chunks.py** | BERT 청크 확인 | `python scripts/debug/check_bert_chunks.py` |
| **test_summarize_debug.py** | 요약 디버깅 | `python scripts/debug/test_summarize_debug.py` |

#### 4.5 scripts/ (루트)

| 파일 | 설명 | 실행 명령 |
|------|------|----------|
| **evaluate_answers.py** | 답변 평가 스크립트 (LLM-as-a-Judge) | `python scripts/evaluate_answers.py` |

---

### 5. data/ - 데이터 디렉토리

| 디렉토리 | 설명 |
|----------|------|
| **raw/pdfs/** | 원본 PDF 파일 (논문) |
| **processed/** | 전처리된 데이터 (청킹된 텍스트) |
| **rdbms/** | RDBMS 데이터 (PostgreSQL 덤프 등) |
| **vectordb/papers_faiss/** | FAISS 벡터 인덱스 |

---

### 6. configs/ - 설정 파일

| 디렉토리 | 설명 |
|----------|------|
| **collect/** | 논문 수집 설정 (YAML) |

---

### 7. logs/ - 로그 파일

| 디렉토리 | 설명 |
|----------|------|
| **experiments/YYYYMMDD_HHMMSS/** | 실험별 로그 디렉토리 |

**로그 파일 구조 예시:**
```
logs/experiments/20251104_143022/
├── experiment_metadata.json    # 실험 메타데이터
├── agent.log                   # Agent 로그
├── router.log                  # Router 로그
├── general_answer.log          # 일반 답변 로그
├── search_paper.log            # 논문 검색 로그
└── ...
```

---

### 8. notebooks/ - Jupyter 노트북

| 디렉토리 | 설명 |
|----------|------|
| **base/** | 기본 노트북 (실험, 프로토타이핑) |
| **team/** | 팀원별 노트북 |

---

### 9. docs/ - 문서

#### 9.1 docs/modularization/ - 모듈화 문서

| 파일 | 설명 |
|------|------|
| **00_프로젝트_구조.md** | 프로젝트 구조 (본 문서) |
| **01_개발환경_및_기술스택.md** | 개발 환경 및 기술 스택 |
| **01_로깅_시스템.md** | 로깅 시스템 |
| **02_실험_관리_시스템.md** | 실험 관리 시스템 |
| **03_AI_Agent_시스템.md** | AI Agent 시스템 |
| **04_LLM_클라이언트.md** | LLM 클라이언트 |
| **05_대화_메모리_시스템.md** | 대화 메모리 시스템 |
| **05_프롬프트_엔지니어링.md** | 프롬프트 엔지니어링 |
| **06_도구_시스템.md** | 도구 시스템 |
| **06-1_도구_자동전환_기능.md** | 도구 자동전환 및 Fallback Chain 메커니즘 |
| **07_Streamlit_UI_시스템.md** | Streamlit UI 시스템 |
| **08_데이터베이스_시스템.md** | 데이터베이스 시스템 |
| **09_RAG_시스템.md** | RAG 시스템 |
| **10_성능평가시스템.md** | 성능 평가 시스템 |

#### 9.2 docs/PRD/ - PRD 문서

| 파일 | 설명 |
|------|------|
| **01_프로젝트_개요.md** | 프로젝트 개요 |
| **02_유저_페르소나.md** | 유저 페르소나 |
| **03_핵심_기능_명세.md** | 핵심 기능 명세 |
| **04_AI_Agent_아키텍처.md** | AI Agent 아키텍처 |
| **05_RAG_시스템_설계.md** | RAG 시스템 설계 |
| **06_데이터베이스_설계.md** | 데이터베이스 설계 |
| **07_UI_UX_설계.md** | UI/UX 설계 |
| **08_프롬프트_전략.md** | 프롬프트 전략 |
| **09_평가_기준.md** | 평가 기준 |
| **10_기술_요구사항.md** | 기술 요구사항 |

#### 9.3 docs/roles/ - 담당역할 문서

| 파일 | 설명 |
|------|------|
| **담당역할_01_이예포_로깅시스템.md** | 이예포 - 로깅 시스템 |
| **담당역할_02_윤정인_실험관리.md** | 윤정인 - 실험 관리 |
| **담당역할_03_강도형_AI_Agent.md** | 강도형 - AI Agent |
| **담당역할_04_권지원_LLM_클라이언트.md** | 권지원 - LLM 클라이언트 |
| **담당역할_05-1_신준엽_Text-to-SQL.md** | 신준엽 - Text-to-SQL |
| **담당역할_05-2_최현화_성능평가시스템.md** | 최현화 - 성능 평가 시스템 |

#### 9.4 docs/issues/ - 이슈 트래킹 문서

| 파일 | 설명 |
|------|------|
| **05-1_Text-to-SQL_구현.md** | Text-to-SQL 구현 이슈 |
| **05-2_성능평가시스템_구현.md** | 성능 평가 시스템 구현 이슈 |
| **merge/** | 병합 관련 이슈 |

#### 9.5 docs/usage/ - 사용 가이드

| 디렉토리 | 설명 |
|----------|------|
| **team/** | 팀원별 사용 가이드 |
| **tests/** | 테스트 가이드 |

#### 9.6 docs/rules/ - 코딩 규칙

| 파일 | 설명 |
|------|------|
| **annotate_style.md** | 한글 주석 스타일 가이드 |
| **mermaid_style.md** | Mermaid 다이어그램 스타일 가이드 |
| **commit_msg.md** | 커밋 메시지 가이드 |

#### 9.7 docs/minutes/ - 회의록

| 디렉토리 | 설명 |
|----------|------|
| **20251028/** | 2025-10-28 회의록 |
| **20251029/** | 2025-10-29 회의록 |
| **20251030/** | 2025-10-30 회의록 |
| **20251031/** | 2025-10-31 회의록 |
| **20251103/** | 2025-11-03 회의록 |

---

### 10. 기타 디렉토리

| 디렉토리 | 설명 |
|----------|------|
| **.streamlit/** | Streamlit 설정 파일 (config.toml) |
| **.github/ISSUE_TEMPLATE/** | GitHub 이슈 템플릿 |

---

## 모듈 간 의존성

```mermaid
graph TB
    subgraph UI_Layer["🔸 UI 계층"]
        direction LR
        UI[ui/app.py]
    end

    subgraph Business_Layer["🔹 비즈니스 계층"]
        direction LR
        Agent[agent/graph.py<br/>Fallback Chain] --> Tools[tools/*]
        Agent --> Classifier[agent/question_classifier.py]
        Agent --> FailDetector[agent/failure_detector.py]
        Agent --> ConfigLoader[agent/config_loader.py]
        Agent --> ToolWrapper[agent/tool_wrapper.py]
        Tools --> RAG[rag/retriever.py]
    end

    subgraph Data_Layer["🔺 데이터 계층"]
        direction LR
        DB[database/db.py] --> Vector[database/vector_store.py]
        Vector --> Memory[memory/chat_history.py]
    end

    subgraph Infra_Layer["🔶 인프라 계층"]
        direction LR
        LLM[llm/client.py] --> Logger[utils/logger.py]
        Logger --> ExpMgr[utils/experiment_manager.py]
    end

    UI --> Agent
    Agent --> Memory
    Tools --> RAG
    Tools --> LLM
    RAG --> Vector
    Vector --> DB
    Memory --> DB

    Agent --> ExpMgr
    Tools --> Logger
    RAG --> Logger

    %% Subgraph 스타일
    style UI_Layer fill:#e1f5ff,stroke:#01579b,stroke-width:3px,color:#000
    style Business_Layer fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style Data_Layer fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Infra_Layer fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000

    %% 노드 스타일 (UI 계층 - 파랑 계열)
    style UI fill:#90caf9,stroke:#1976d2,color:#000

    %% 노드 스타일 (비즈니스 계층 - 보라 계열)
    style Agent fill:#ce93d8,stroke:#7b1fa2,color:#000
    style Classifier fill:#ba68c8,stroke:#7b1fa2,color:#fff
    style FailDetector fill:#ba68c8,stroke:#7b1fa2,color:#fff
    style ConfigLoader fill:#ba68c8,stroke:#7b1fa2,color:#fff
    style ToolWrapper fill:#ba68c8,stroke:#7b1fa2,color:#fff
    style Tools fill:#ab47bc,stroke:#4a148c,color:#fff
    style RAG fill:#9c27b0,stroke:#4a148c,color:#fff

    %% 노드 스타일 (데이터 계층 - 녹색 계열)
    style DB fill:#a5d6a7,stroke:#388e3c,color:#000
    style Vector fill:#81c784,stroke:#2e7d32,color:#000
    style Memory fill:#66bb6a,stroke:#2e7d32,color:#fff

    %% 노드 스타일 (인프라 계층 - 주황 계열)
    style LLM fill:#ffcc80,stroke:#f57c00,color:#000
    style Logger fill:#ffb74d,stroke:#f57c00,color:#000
    style ExpMgr fill:#ffa726,stroke:#ef6c00,color:#000
```

**의존성 설명:**
- **UI 계층**: Streamlit UI가 Agent를 호출
- **비즈니스 계층**: Agent가 Fallback Chain 관련 모듈을 사용
  - **QuestionClassifier**: 질문 유형 분류 (7가지 유형)
  - **FailureDetector**: 도구 실행 실패 감지
  - **ConfigLoader**: Fallback Chain 설정 로드 (YAML)
  - **ToolWrapper**: 도구 상태 자동 설정
  - Agent가 Tools를 호출하고, Tools가 RAG/LLM/Logger를 사용
- **데이터 계층**: RAG가 VectorDB를 사용하고, Memory가 PostgreSQL 사용
- **인프라 계층**: LLM, Logger, ExperimentManager가 모든 계층에서 사용됨

---

## 핵심 실행 파일

### 1. 메인 진입점

| 파일 | 실행 명령 | 설명 |
|------|----------|------|
| **main.py** | `python main.py` | AI Agent 실행 (CLI) |
| **ui/app.py** | `streamlit run ui/app.py` | Streamlit UI 실행 |

### 2. 데이터 파이프라인

| 실행 순서 | 파일 | 실행 명령 |
|----------|------|----------|
| **1** | scripts/data/setup_database.py | `python scripts/data/setup_database.py` |
| **2** | scripts/data/collect_arxiv_papers.py | `python scripts/data/collect_arxiv_papers.py` |
| **3** | scripts/data/process_documents.py | `python scripts/data/process_documents.py` |
| **4** | scripts/data/load_embeddings.py | `python scripts/data/load_embeddings.py` |

### 3. 테스트 실행

| 파일 | 실행 명령 |
|------|----------|
| **전체 단위 테스트** | `pytest scripts/tests/unit/` |
| **전체 통합 테스트** | `pytest scripts/tests/integration/` |
| **특정 테스트** | `python scripts/tests/unit/test_db_connection.py` |

---

## 환경 변수 설정

### .env 파일 구조

```bash
# ==================== OpenAI API ====================
OPENAI_API_KEY=sk-your-openai-api-key-here

# ==================== Solar API (Upstage) ====================
SOLAR_API_KEY=up-your-solar-api-key-here

# ==================== PostgreSQL ====================
POSTGRES_USER=langchain_user
POSTGRES_PASSWORD=your_password
POSTGRES_HOST=localhost
POSTGRES_PORT=5432
POSTGRES_DB=papers

# ==================== Tavily Search API ====================
TAVILY_API_KEY=tvly-your-tavily-api-key-here

# ==================== 개발 환경 ====================
ENVIRONMENT=development  # development, production
LOG_LEVEL=INFO          # DEBUG, INFO, WARNING, ERROR

# ==================== LLM 설정 ====================
LLM_MODEL=gpt-5  # GPT-5 모델 사용
LLM_TEMPERATURE=0.7
LLM_MAX_TOKENS=2000

# ==================== Solar LLM 설정 ====================
SOLAR_MODEL=solar-pro2  # Solar Pro2 모델 사용
SOLAR_TEMPERATURE=0.7
SOLAR_MAX_TOKENS=2000

# ==================== Embeddings 설정 ====================
EMBEDDING_MODEL=text-embedding-3-small
EMBEDDING_DIMENSION=1536
```

---

## Git 관리

### .gitignore 주요 내용

```
# 환경 변수
.env

# Python 캐시
__pycache__/
*.pyc
*.pyo
*.pyd

# 가상 환경
venv/
.venv/

# 데이터
data/raw/pdfs/*.pdf
data/vectordb/papers_faiss/*

# 로그
logs/experiments/*

# IDE
.vscode/
.idea/

# OS
.DS_Store
Thumbs.db
```

---

## 프로젝트 확장 가이드

### 새로운 도구 추가하기

1. **src/tools/** 디렉토리에 새로운 도구 파일 생성
   - 파일명: `<tool_name>.py`
   - 함수명: `<tool_name>(state: AgentState) -> dict`

2. **src/tools/__init__.py**에 도구 임포트 추가
   ```python
   from .new_tool import new_tool
   ```

3. **src/agent/graph.py**에서 도구 등록
   ```python
   tools = [general_answer, search_paper, summarize, glossary, web_search, text2sql, new_tool]
   ```

4. **docs/modularization/06_도구_시스템.md** 문서 업데이트

### 새로운 테스트 추가하기

1. **scripts/tests/unit/** 또는 **scripts/tests/integration/**에 테스트 파일 생성
   - 파일명: `test_<module_name>.py`

2. pytest 실행
   ```bash
   pytest scripts/tests/unit/test_<module_name>.py -v
   ```

### 새로운 모듈화 문서 추가하기

1. **docs/modularization/** 디렉토리에 새로운 문서 생성
   - 파일명: `<번호>_<모듈명>.md`
   - 예: `11_새로운_모듈.md`

2. 문서 형식은 기존 문서와 동일하게 유지
   - 문서 정보, 개요, 시스템 아키텍처, 구현 파일 구조, 주요 클래스/함수, 사용 예시 등

---

## 참고 문서

- [docs/modularization/01_개발환경_및_기술스택.md](./01_개발환경_및_기술스택.md) - 개발 환경 설정
- [docs/PRD/04_AI_Agent_아키텍처.md](../PRD/04_AI_Agent_아키텍처.md) - AI Agent 아키텍처 설계
- [docs/PRD/10_기술_요구사항.md](../PRD/10_기술_요구사항.md) - 기술 스택 상세 명세
- [docs/rules/annotate_style.md](../rules/annotate_style.md) - 한글 주석 스타일 가이드
- [docs/rules/mermaid_style.md](../rules/mermaid_style.md) - Mermaid 다이어그램 스타일 가이드

---

## 작성자

- **최현화[팀장]** (프로젝트 구조 문서화)
