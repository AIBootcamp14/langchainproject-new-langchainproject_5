...
graph TB
    subgraph MainFlow["📋 AI Agent 질문 처리 워크플로우"]
        ...
        subgraph AgentProcessing["🔹 Agent 라우팅 (Fallback Chain 지원)"]
            direction LR
            Difficulty --> ClassifyQ[질문 유형 분류<br/>7가지 유형]
            ClassifyQ --> Router{라우터 노드<br/>질문 분석}
            ...
            GeneralTool -.->|실패 시| FallbackRouter[Fallback Router<br/>다음 도구 선택]
            RAGTool -.->|실패 시| FallbackRouter
            WebTool -.->|실패 시| FallbackRouter
            GlossaryTool -.->|실패 시| FallbackRouter
            SummarizeTool -.->|실패 시| FallbackRouter
            Text2SQLTool -.->|실패 시| FallbackRouter
            FallbackRouter -.->|재시도| Router
        end
    ...
    %% 노드 스타일 (Agent - 보라 계열)
    style ClassifyQ fill:#ba68c8,stroke:#6a1b9a,stroke-width:2px,color:#fff
    ...
    style FallbackRouter fill:#ab47bc,stroke:#4a148c,stroke-width:2px,color:#fff
...
**사용자 질문 처리 흐름 설명:**
- **질문 입력 단계**: 사용자가 질문을 입력하고 Easy/Hard 난이도를 선택하여 답변 수준 결정
- **Agent 라우팅 단계 (Fallback Chain 지원)**:
  - **질문 유형 분류**: QuestionClassifier가 질문을 7가지 유형으로 자동 분류
    - term_definition (용어 정의)
    - paper_search (논문 검색)
    - latest_research (최신 연구)
    - paper_summary (논문 요약)
    - statistics (통계 정보)
    - general_question (일반 질문)
    - file_save (파일 저장)
  - **라우터 노드**: 질문 유형에 따라 7개 도구 중 적절한 도구 선택
    - 일반 답변 (General Answer)
    - RAG 검색 (논문 DB 검색)
    - 웹 검색 (Tavily API)
    - 용어집 (Glossary)
    - 논문 요약 (Summarize)
    - Text-to-SQL (통계 조회)
    - 파일 저장 (Save File)
  - **Fallback Router** (신규): 도구 실행 실패 시 자동으로 다음 우선순위 도구로 전환
    - 예: glossary 실패 → general 자동 실행
    - 예: search_paper 실패 → web_search → general
- **데이터 조회 단계**: 선택된 도구에 따라 다양한 데이터 소스 조회
  - LLM 직접 호출 (일반 답변)
  - PGVector (논문 임베딩 검색)
  - PostgreSQL (용어집, papers 테이블)
  - Tavily API (웹 검색)
- **최종 답변 생성 단계**: LLM (GPT-5 또는 Solar Pro2)이 난이도에 맞는 답변 생성
- **답변 표시 단계**: 채팅 화면에 답변 표시 후 용어 자동 추출 및 DB 저장
- **품질 평가 단계**: LLM-as-a-Judge 방식으로 답변 품질을 실시간 평가
  - 정확도 (0-10점): 참고 문서와의 일치도
  - 관련성 (0-10점): 질문과 답변의 연관성
  - 난이도 적합성 (0-10점): 선택한 난이도에 맞는 답변인지
  - 출처 명시 (0-10점): 논문 제목, 저자 명시 여부
  - 평가 결과를 DB에 자동 저장하고 UI에 expander로 표시

> **참고**:
> - 성능 평가는 답변 생성 후 실시간으로 자동 실행되며, Streamlit UI는 평가 완료 후 자동으로 입력창으로 복귀하여 연속 질문이 가능합니다.
> - Fallback Chain 메커니즘은 `configs/model_config.yaml`에서 활성화/비활성화 가능합니다. (기본값: 활성화)
> - 상세한 Fallback Chain 동작 방식은 **[09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md)** 문서를 참고하세요.
