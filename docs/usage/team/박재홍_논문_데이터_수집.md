...
PostgreSQL에 저장되는 pgvector 테이블은 **LangChain 라이브러리가 자동으로 생성**하는 테이블입니다:

- **`langchain_pg_collection`**: 벡터 컬렉션 메타데이터 테이블
  - 저장 내용: 컬렉션 이름 (`paper_chunks`), UUID, 메타데이터
  - 역할: 여러 벡터 컬렉션을 논리적으로 구분

- **`langchain_pg_embedding`**: 실제 임베딩 벡터 저장 테이블
  - 저장 내용: 임베딩 벡터 (1536차원), 원본 텍스트, 메타데이터
  - 역할: 논문 청크의 벡터 임베딩 및 원본 문서 저장

**❗ 주의:** 이 테이블명은 LangChain 라이브러리가 하드코딩으로 정의한 것으로, **변경할 수 없습니다**. 박재홍 팀원이 설정한 것이 아닙니다.
...
박재홍 팀원이 구현한 **실험용 Streamlit UI**를 실행할 수 있습니다.

### 특징

**기술 스택:**
- UI: Streamlit
- Vector DB: FAISS (로컬 파일)
- RDBMS: SQLite (로컬 파일)
- LLM: OpenAI ChatGPT (gpt-5o-mini)
- Embeddings: HuggingFace (sentence-transformers/all-MiniLM-L6-v2)

**주요 기능:**
1. **검색/색인**: arXiv 논문 검색 및 벡터 DB 저장
2. **용어집**: AI/ML 용어 자동 추출
3. **RAG Q&A**: EZ(초심자)/HARD(대학원) 모드 질의응답
...
```bash
OPENAI_API_KEY=sk-...
OPENAI_MODEL=gpt-5o-mini  # 선택사항 (기본값: gpt-5o-mini)
```
...
브라우저에서 `http://localhost:8501` 접속하여 UI 사용
...
| 항목 | Streamlit UI (실험) | 프로덕션 파이프라인 |
|-----|-------------------|-------------------|
| Vector DB | FAISS (로컬) | PostgreSQL + pgvector |
| RDBMS | SQLite (로컬) | PostgreSQL |
| Embeddings | HuggingFace (무료) | OpenAI (유료) |
| 용도 | 실험/데모 | 프로덕션 |

**2. OpenAI API 키 필수**
- Streamlit UI는 OpenAI API 키가 반드시 필요합니다
- `.env` 파일에 올바른 API 키 설정 확인

**3. 로컬 데이터**
- FAISS와 SQLite는 로컬 파일로 저장됩니다
- 프로덕션 파이프라인의 PostgreSQL 데이터와는 별개입니다
- UI에서 저장한 데이터는 프로덕션 DB에 반영되지 않습니다

**4. 포트 충돌**
- 기본 포트: 8501
- 포트 변경: `streamlit run ui/test/paper_review_app.py --server.port 8502`
