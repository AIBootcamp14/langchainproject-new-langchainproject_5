...
**파일:** `src/rag/retriever.py`, `src/tools/rag_search.py`
...
| 단계 | 동작 |
|------|------|
| 1 | OpenAIEmbeddings 초기화 (model="text-embedding-3-small") |
| 2 | PGVector 초기화 (collection_name="paper_chunks") |
| 3 | base_retriever 설정 (search_type="mmr", k=5, fetch_k=20, lambda_mult=0.5) |
| 4 | ChatOpenAI 초기화 (model="gpt-5", temperature=0) |
| 5 | MultiQueryRetriever.from_llm() 생성 |

**메서드:**

| 메서드 | 파라미터 | 동작 |
|--------|---------|------|
| retrieve | query: str, use_multi_query: bool = True | use_multi_query에 따라 multi_query_retriever 또는 base_retriever로 검색 |
| retrieve_with_filter | query: str, filter_dict: dict | 메타데이터 필터링 포함 검색 (similarity search) |
| retrieve_with_scores | query: str | 유사도 점수 포함 검색 (similarity_search_with_score) |

**함수: search_paper_database (@ tool 데코레이터)**

| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| query | str | (필수) | 검색할 질문 또는 키워드 |
| year_filter | int | None | 선택적 년도 필터 |
...
|------|------|
| 1 | RAGRetriever 초기화 |
| 2 | year_filter 있으면 retrieve_with_filter() 사용, 없으면 retrieve() 사용 |
| 3 | psycopg2로 PostgreSQL 연결 |
| 4 | 각 검색된 문서에서 paper_id 추출 |
| 5 | papers 테이블에서 title, authors, publish_date, url 조회 |
| 6 | results 리스트 구성 (title, authors, publish_date, url, content, section) |
| 7 | exp_manager.tool_logger로 검색 결과 로깅 |
| 8 | format_search_results()로 Markdown 포맷팅 후 반환 |

**함수: format_search_results**

- 검색 결과를 Markdown 형식으로 포맷팅
- 각 논문마다 제목, 저자, 출판일, URL, 섹션, 내용(처음 500자)을 포함
- 결과가 없으면 "관련 논문을 찾을 수 없습니다." 반환
...
**파일:** `src/rag/glossary_retriever.py`, `src/tools/glossary.py`, `src/rag/context_enhancer.py`
...
|------|------|
| 1 | OpenAIEmbeddings 초기화 (model="text-embedding-3-small") |
| 2 | PGVector 초기화 (collection_name="glossary_embeddings") |
| 3 | retriever 설정 (search_type="similarity", k=3) |

**메서드:**

| 메서드 | 파라미터 | 동작 |
|--------|---------|------|
| search | term: str | retriever.invoke(term)로 용어 검색 후 결과 반환 |

**함수: search_glossary (@tool 데코레이터)**

| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| term | str | (필수) | 검색할 용어 |
| difficulty | str | "easy" | 'easy' 또는 'hard' |
| exp_manager | ExperimentManager | None | 실험 관리자 인스턴스 |
...
| 단계 | 검색 대상 | 동작 |
|------|----------|------|
| 1 | PostgreSQL glossary 테이블 | ILIKE로 용어 검색, 결과 있으면 난이도별 설명 반환 |
| 2 | Vector DB (glossary_embeddings) | GlossaryRetriever로 유사 용어 검색, 결과 있으면 반환 |
| 3 | 논문 본문 (paper_chunks) | search_paper_database()로 논문에서 용어 정의 검색 |

**함수: extract_and_add_glossary_context**

| 파라미터 | 타입 | 기본값 | 설명 |
|---------|------|--------|------|
| user_query | str | (필수) | 사용자 질문 |
| difficulty | str | "easy" | 난이도 |
| exp_manager | ExperimentManager | None | 실험 관리자 인스턴스 |
...
| 단계 | 동작 |
|------|------|
| 1 | psycopg2로 PostgreSQL 연결 |
| 2 | ILIKE를 사용하여 질문 내 포함된 용어 검색 |
| 3 | 용어 발견 시 Markdown 형식으로 "관련 용어 정의" 섹션 구성 |
| 4 | exp_manager.logger로 추출된 용어 개수 로깅 |
| 5 | 용어 없으면 빈 문자열 반환 |
