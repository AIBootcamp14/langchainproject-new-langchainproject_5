...
|------|-----------|-----------|-------------|
| **Glossary Node** | ✅ 적용 완료 (2025-11-04) | - | - |
| **Search Paper Node** | ✅ 적용 완료 (2025-11-04) | - | - |
| **Save File Node** | ❌ 미적용 (선택 사항) | 메시지 템플릿 적용 | ⭐ 쉬움 |

**Save File 노드 작업 방법 (선택 사항)**:
- 현재 Save File 노드는 단순 파일 저장 기능만 수행
- 프롬프트 적용이 필수는 아니지만, 사용자 친화적 메시지를 위해 적용 가능
- 우선순위: 낮음
...
**권장 사항**:
1. `configs/prompt_config.yaml` 파일을 deprecated로 표시
2. 파일 최상단에 주석 추가:
   ```yaml
   # ⚠️ DEPRECATED: 이 파일은 더 이상 사용되지 않습니다.
   # 현재 prompts/*.json 파일들을 사용하고 있습니다.
   # 참고: src/prompts/loader.py
   ```
3. 또는 삭제 후 `configs/prompt_config.yaml.backup`으로 백업
