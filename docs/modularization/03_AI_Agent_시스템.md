...
AI Agent 시스템은 **LangGraph StateGraph**를 기반으로 **사용자 질문을 분석하고 적절한 도구를 자동으로 선택하여 실행하는 지능형 시스템**입니다. `src/agent/` 폴더에 구현되어 있으며, 라우터 노드와 6개의 도구 노드로 구성되어 있습니다.

### 주요 역할

1. **질문 분석**: 라우터 노드에서 LLM을 활용하여 질문 유형 분석
2. **도구 선택**: 6가지 도구 중 질문에 가장 적합한 도구 자동 선택
...
├── nodes.py              # 7개 노드 함수 (router + 6 tools)
└── __init__.py           # 모듈 초기화
```
...
### 1. 라우터 노드 (Router Node)

**역할:**
- 사용자 질문을 LLM으로 분석
- 6가지 도구 중 가장 적합한 도구 선택
...
**사용 방식:**
```python
# 대화 히스토리를 state["messages"]에 전달
state = {
    "question": "그럼 RAG는 어디에 사용돼?",
...
- **[06_도구_시스템.md](./06_도구_시스템.md)** - 6가지 도구 상세
...
3. ✅ 6가지 도구 노드 (general, search_paper, web_search, glossary, summarize, save_file)
