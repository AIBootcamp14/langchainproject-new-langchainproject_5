...
4. **긴 컨텍스트 윈도우**
   - GPT-5: 128K 토큰 (논문 전체 처리 가능)

#### 단점
1. **비용**
   - GPT-5: $0.04/1K tokens (input), $0.08/1K tokens (output)
   - 사용량에 따라 비용 증가
...
**필요 라이브러리:** `langchain.vectorstores.PGVector`

**PGVector 인스턴스 생성 파라미터:**

| 파라미터 | 값 예시 | 설명 |
|---------|---------|------|
| connection_string | `postgresql://user:password@localhost:5432/papers` | PostgreSQL 연결 문자열 |
| embedding_function | embeddings 객체 | OpenAI Embeddings 인스턴스 |
| collection_name | `"paper_embeddings"` | 벡터 컬렉션 이름 |

**사용 방법:** PostgreSQL + pgvector를 Vector Store로 사용하여 논문 임베딩을 저장하고 검색
...
| 컴포넌트 | 기능 | 설명 |
|---------|------|------|
| st.title | 페이지 제목 | "논문 리뷰 챗봇" 표시 |
| st.selectbox | 난이도 선택 | "Easy 모드 (초심자)", "Hard 모드 (전문가)" 선택 |
| st.session_state | 상태 관리 | messages 리스트로 채팅 히스토리 저장 |
| st.chat_message | 메시지 표시 | 사용자/AI 메시지를 채팅 형식으로 표시 |
| st.chat_input | 사용자 입력 | "논문에 대해 질문해주세요" 입력창 |
| st.markdown | 내용 렌더링 | 메시지 내용을 Markdown 형식으로 표시 |

**처리 흐름:**
1. 세션 상태 초기화 (messages 리스트)
2. 기존 메시지 히스토리 표시
3. 사용자 입력 수신 및 히스토리에 추가
4. Agent 호출 (question, difficulty 전달)
5. AI 응답 표시 및 히스토리 업데이트
...
| chunk_overlap | 200 | 청크 간 중복 문자 수 (컨텍스트 유지) |
| separators | `["\n\n", "\n", ". ", " ", ""]` | 청크 분할 우선순위 (단락 → 문장 → 단어) |

**동작 방식:** 긴 논문 텍스트를 의미적으로 연관된 작은 청크로 분할하여 임베딩 및 검색 효율성 향상
...
| 구분 | 선택 기술 | 이유 |
|------|----------|------|
| **LLM API** | OpenAI (GPT-5) + Solar (Solar Pro2) | 높은 성능과 정확도, 안정적인 Function Calling |
