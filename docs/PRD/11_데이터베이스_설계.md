...
**필요 라이브러리:**
- `langchain_postgres.vectorstores.PGVector` - PostgreSQL 벡터 스토어
...
| 설정 항목 | 값 | 설명 |
|----------|-----|------|
| 클래스 | OpenAIEmbeddings | OpenAI 임베딩 모델 |
| model | text-embedding-3-small | 임베딩 모델명 |
| openai_api_key | 환경 변수 OPENAI_API_KEY | API 키 (환경 변수에서 로드) |
...
| 컬렉션명 | collection_name | 용도 | embedding_function | connection_string |
|---------|----------------|------|-------------------|------------------|
| paper_chunks_store | "paper_chunks" | 논문 본문 청크 임베딩 | embeddings (OpenAIEmbeddings) | CONNECTION_STRING |
| abstract_store | "paper_abstracts" | 논문 초록 임베딩 | embeddings (OpenAIEmbeddings) | CONNECTION_STRING |
| glossary_store | "glossary_embeddings" | 용어집 임베딩 | embeddings (OpenAIEmbeddings) | CONNECTION_STRING |

**PGVector 인스턴스 생성 파라미터:**

| 파라미터 | 타입 | 설명 |
|---------|------|------|
| collection_name | str | 컬렉션 이름 (테이블명) |
| embedding_function | OpenAIEmbeddings | 임베딩 생성 함수 |
...
| 컬렉션 | 검색 대상 | 사용 도구 |
|--------|----------|----------|
| paper_chunks | 논문 본문 상세 검색 | RAG 검색 도구 |
| paper_abstracts | 논문 초록 검색 (빠른 개요) | 논문 요약 도구 |
| glossary_embeddings | 유사 용어 검색 | 용어집 도구 |
