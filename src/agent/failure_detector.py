...
class FailureDetector:
    ...
    REGEX_PATTERNS = [
        ...
        # "결과가 없" 패턴 제거 - text2sql의 빈 결과셋과 구분 불가능
        # 대신 정확한 문자열 매칭 (FAILURE_PATTERNS)만 사용
        r".*오류가?\s*(발생|났|생겼).*",                # "오류가 발생", "오류 발생" 패턴 (부정적 맥락)
        r".*중\s*오류.*",                               # "실행 중 오류", "처리 중 오류" 패턴
        r".*실패(했|됐|됨|함).*",                       # "실패했", "실패됨" 패턴 (부정적 맥락)
        r".*에\s*실패.*",                               # "생성에 실패", "조회에 실패" 패턴
        r".*에러.*:.*SQL.*",                            # "에러: SQL 실행 실패" 패턴 (SQL 에러만)
        r"ERROR.*SQL.*",                                # "ERROR SQL" 패턴 (SQL 에러만)
