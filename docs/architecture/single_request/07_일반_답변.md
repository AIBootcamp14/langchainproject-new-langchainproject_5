...
| 6 | `prompts/routing_prompts.json` | - | LLM 라우팅 (패턴 실패 시) | question | tool_choice | LLM (GPT-5) |
...
|--------|--------|--------|----------|-----|------|
| **easy** | elementary | beginner | Solar-pro2 | 매우 친근하고 쉬운 설명 | 초등학생~대학생 |
| **hard** | intermediate | advanced | GPT-5 | 기술적이고 학술적인 설명 | 중급자~연구자 |
...
**모델 매핑**:
- `difficulty="easy"` → `solar-pro2` (빠르고 경제적)
- `difficulty="hard"` → `gpt-5` (더 정확하고 상세한 답변)
...
| 모델 | 난이도 | 속도 | 비용 | 정확도 | 용도 |
|------|--------|------|------|--------|------|
| **Solar-pro2** | easy | 빠름 | 저렴 | 충분 | 일반 질문, 간단한 설명 |
| **GPT-5** | hard | 느림 | 비쌈 | 높음 | 복잡한 설명, 학술적 답변 |
