...
│
└── 📁 pgvector 테이블 (LangChain 자동 생성, 2개)
...
### 4. langchain_pg_collection (pgvector 컬렉션 메타데이터)
...
**컬렉션 목록**:
- `paper_chunks`: 논문 본문 청크 임베딩
- (향후 확장 가능: paper_abstracts, glossary_embeddings)

---

### 5. langchain_pg_embedding (pgvector 벡터 데이터)
...
**pgvector 테이블** (LangChain, 2개):
- langchain_pg_collection (컬렉션)
