...
용어집 검색 도구와 별개로, **다른 도구들의 LLM 답변에서 AI/ML 용어를 자동으로 추출하여 glossary 테이블에 저장**하는 기능이 있습니다.

**처리 흐름:**
```
도구 실행 → LLM 답변 생성 → 용어 추출 대상 도구인지 확인
    ↓
(대상 도구인 경우)
    ↓
답변 텍스트 → LLM 용어 추출 → 정의/설명 생성 → glossary 테이블 저장
```
...
| 도구 | 용어 추출 | 이유 |
|------|----------|------|
| `general_answer` | ✅ 실행 | AI/ML 개념 설명 포함 가능성 높음 |
| `search_paper` | ✅ 실행 | 논문 설명에 전문 용어 다수 포함 |
| `web_search` | ✅ 실행 | 최신 AI/ML 용어 포함 가능 |
| `glossary` | ✅ 실행 | 용어 정의 답변 자체가 용어 |
| `summarize` | ✅ 실행 | 논문 요약에 전문 용어 포함 |
| **`text2sql`** | **❌ 제외** | **SQL 쿼리와 통계 결과만 포함 (용어 없음)** |
| **`save_file`** | **❌ 제외** | **파일 저장 완료 메시지만 반환 (용어 없음)** |

**제외 이유:**

- **text2sql**: 답변이 "생성된 SQL: `SELECT COUNT(*) ...`" + "결과: 42" 형태로 AI/ML 용어가 전혀 없음
- **save_file**: 답변이 "파일 저장 완료" 메시지뿐

**구현 위치:**

- **파일**: `ui/components/chat_interface.py` (298-318 라인)
- **함수**: `handle_agent_response()`에서 도구 실행 후 자동 호출
...
1. LLM에게 답변 텍스트 전달
2. AI/ML 관련 전문 용어 식별 (최대 5개)
3. 각 용어별로 생성:
   - `term`: 용어명
   - `definition`: 간단한 정의 (1-2문장)
   - `easy_explanation`: 초보자용 쉬운 설명 (비유/예시 포함)
   - `hard_explanation`: 전문가용 상세 설명 (수식/기술 세부사항)
   - `category`: 카테고리 (예: Deep Learning, NLP)
4. PostgreSQL glossary 테이블 저장 (중복 체크: `ON CONFLICT (term) DO NOTHING`)

**장점:**

- 자동으로 용어집 DB 확장
- 향후 glossary 도구 검색 시 활용
- 불필요한 LLM 호출 방지 (text2sql, save_file 제외)
- 비용 절감

**관련 파일:**

- `src/utils/glossary_extractor.py`: 용어 추출 함수 (`extract_and_save_terms()`)
...
## 🔧 도구 5: 논문 요약 (Summarize) - 2025-11-04 개선
...
### summary.md 저장 (2025-11-04 개선)

**개선 내용**:
- ✅ **저장 위치 변경**: `outputs/summary.md` → `outputs/summary/{논문제목}.md`
- ✅ **논문 제목을 파일명으로 사용**: 특수문자 제거 및 최대 100자 제한
- ✅ **summary 폴더 자동 생성**: `outputs/summary/` 폴더 자동 생성

**구현 코드** (`src/tools/summarize.py:255-268`):
```python
...
└── summary/
    ...
    ├── BERT_Pre-training_of_Deep_Bidirectional_Transformers.md
    └── GPT-4_Technical_Report.md
```

**변경 전후 비교**:
- **변경 전**: `outputs/summary.md` (하나의 파일에 덮어쓰기)
- **변경 후**: `outputs/summary/{논문제목}.md` (논문별 개별 파일)

**장점**:
- 여러 논문 요약 시 각각 독립 파일로 저장
- 파일명만 봐도 어떤 논문인지 확인 가능
- 요약 내역 관리 용이
...
class AgentState(TypedDict):
    ...
    # Fallback Chain 필드 (선택적)
    retry_count: int              # 재시도 횟수
    failed_tools: List[str]       # 실패한 도구 리스트
    question_type: str            # 질문 유형 (7가지)
...
- **fallback_router** (신규): 도구 실패 시 다음 도구 선택
- **validator** (신규): Router 선택 검증
- **final_fallback** (신규): 최종 Fallback (general 강제 실행)

**엣지 연결:**
```python
# Fallback Chain 비활성화 시 (기존 동작)
...
# Fallback Chain 활성화 시 (신규)
# Router → Validator → 도구 실행 → Fallback 체크 → Fallback Router
```
...
### Fallback Chain 메커니즘 (신규)

**개요:**
도구 실행 실패 시 자동으로 다음 우선순위 도구로 전환하여 사용자 질문에 항상 적절한 답변을 제공하는 메커니즘입니다.
...
**7가지 질문 유형 및 우선순위:**
- **term_definition** (용어 정의): `glossary → general`
- **paper_search** (논문 검색): `search_paper → web_search → general`
- **latest_research** (최신 연구): `web_search → search_paper → general`
- **paper_summary** (논문 요약): `summarize → search_paper → general`
- **statistics** (통계 정보): `text2sql → general`
- **general_question** (일반 질문): `general`
- **file_save** (파일 저장): `save_file`

**설정 파일 (configs/model_config.yaml):**
```yaml
fallback_chain:
  enabled: true              # Fallback Chain 활성화
  max_retries: 3             # 최대 재시도 횟수
  validation_enabled: true   # Router 검증 활성화
  validation_retries: 2      # 검증 재시도 횟수
  priorities:
    term_definition:
      - glossary
      - general
...
Router: glossary 선택
    ↓
Glossary 실행: "관련 용어를 찾을 수 없습니다." (실패)
    ↓
Fallback Router: 다음 도구 선택 (general)
    ↓
General 실행: "ML은 Machine Learning의 약자로..." (성공)
    ↓
END
```

**구현 파일:**
- `src/agent/graph.py` - Fallback Chain 그래프 구성
...
- `src/agent/tool_wrapper.py` - 도구 상태 자동 설정

**상세 문서:**
- **[09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md)** - Fallback Chain 상세 설명
...
- **[09-1_도구_자동전환_기능.md](./09-1_도구_자동전환_기능.md)** - Fallback Chain 메커니즘 상세 설명
