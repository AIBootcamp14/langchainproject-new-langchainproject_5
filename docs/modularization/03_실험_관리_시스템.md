...
#### 4. 전체 대화 자동 저장 ✅
- **파일**: `src/utils/experiment_manager.py:277-289`, `ui/components/chat_interface.py:292-296`
- **저장 위치**: `outputs/conversation_YYYYMMDD_HHMMSS.json`
- **구현 상세**:
  - 매 답변 생성 후 전체 대화 내역 저장
  - Streamlit 세션 상태에서 현재 메시지 추출
  - JSON 형식으로 타임스탬프 포함 저장

```python
# ui/components/chat_interface.py:292-296
if exp_manager:
...
graph TB
    subgraph Init["🔸 초기화 & 폴더 생성"]
        ...
        C --> D[서브 폴더 7개 생성<br/>tools, database, prompts, ui, outputs, evaluation, debug]
