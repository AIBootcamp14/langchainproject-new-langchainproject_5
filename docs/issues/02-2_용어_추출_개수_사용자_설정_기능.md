# 제목: 용어 추출 개수 사용자 설정 기능

---

## 📋 작업 개요
- **작업 주제:** 용어 추출 개수 사용자 설정 기능
- **작성자:** 최현화[팀장]
- **담당자:** @최현화
- **마감일:** 2025-11-04
...
## 📌 이슈 목적
Streamlit UI에서 사용자가 용어 추출 개수를 선택할 수 있도록 기능을 추가합니다.
용어집 용어는 반드시 IT 용어만 등록되도록 조건을 강화합니다.

사용자가 5-10개 사이로 용어 추출수를 지정하더라도 LLM의 답변에서 IT 용어의 개수가 3개로 판별되어 사용자가 지정한 최소 개수 5개보다 적을 경우 최대값인 3개의 용어를 저장합니다.

사용자가 0-5개 사이로 용어 추출수를 지정할 경우, LLM의 답변에서 IT 용어로 판별된 용어의 개수가 10개라면 용어를 다음 기준으로 우선순위를 정합니다.
...
사용자 질문에 대한 LLM 답변에서 해당 용어의 우선순위 높은 순서로 정렬합니다.
정렬 후 최상위부터 사용자가 지정한 개수 만큼 용어를 추출합니다.
