...
- [🔧 핵심 기술 구현](#-핵심-기술-구현)
  ...
  - [6. RAG 시스템](#6-rag-시스템)
  - [7. 도구 시스템 (7가지 Agent 도구)](#7-도구-시스템-7가지-agent-도구)
    - [7-1. RAG 용어집 검색 도구](#7-1-rag-용어집-검색-도구)
    - [7-2. RAG 논문 검색 도구](#7-2-rag-논문-검색-도구)
    - [7-3. WEB 논문 검색 도구](#7-3-web-논문-검색-도구)
    - [7-4. 논문 요약 도구](#7-4-논문-요약-도구)
    - [7-5. Text2SQL 통계 도구](#7-5-text2sql-통계-도구)
    - [7-6. 일반 답변 도구](#7-6-일반-답변-도구)
    - [7-7. 파일 저장 도구](#7-7-파일-저장-도구)
  - [8. 평가 시스템 (LLM-as-a-Judge)](#8-평가-시스템-llm-as-a-judge)
  - [9. 프롬프트 엔지니어링](#9-프롬프트-엔지니어링)
  - [10. Streamlit UI 시스템](#10-streamlit-ui-시스템)
- [📦 설치 및 실행](#-설치-및-실행)
- [⚡ 성능 최적화](#-성능-최적화)
- [📊 주요 성과](#-주요-성과)
- [📚 참고 자료](#-참고-자료)
...
|:----:|------|------------------|----------------------|
| <div align="center"><img src="https://avatars.githubusercontent.com/u/166781765?v=4" width="80" height="80" style="border-radius: 50%;"><br>**[최현화](https://github.com/iejob/langchain-project)**</div> | - Project Lead<br>- Tech Lead | - 프로젝트 총괄 및 시스템 아키텍처 설계<br>- 로깅 & 실험 모니터링 시스템<br>- AI Agent 시스템(LangGraph)<br>- 단일/다중 요청시 Fallback 도구 자동 전환<br>- 멀티턴 대화 기능<br>- Web 논문 검색 도구<br>- 논문 요약 도구<br>- 파일 저장 도구<br>- 평가 시스템(LLM-as-a-Judge)<br>- Streamlit UI | - 팀 리드 및 역할 배분<br>- 개발 규칙 및 컨벤션 정립<br>- 회의록(팀/멘토링) 작성 및 관리<br>- PRD 및 기술 명세 문서 총괄<br>- 프로젝트 전체 문서 작성 및 관리<br>(docs/ 전체, PPT, README)<br>- Github Issue 작성 및 관리<br>- Github Project 관리<br>- Git 브랜치 관리 & 병합 |
...
#### 전체 아키텍처 구조 (상세 흐름도)
![전체 아키텍처 (상세 흐름도)](docs/images/architecture/01-1_전체%20아키텍처(상세%20흐름도).png)
...
![로깅 시스템 아키텍처](docs/images/architecture/02-1_로깅_시스템.png)
...
**실험 폴더 자동 관리 시스템 아키텍처**:

![실험 폴더 자동 관리 시스템 아키텍처](docs/images/architecture/02-2_실험_폴더_자동_관리_시스템.png)
...
PostgreSQL glossary 테이블에서 AI/ML 용어를 검색하고 **난이도별 설명**(Easy/Hard)을 제공하는 도구입니다.

LLM(Solar Pro2)이 질문에서 용어를 자동 추출하여 검색하며, 용어가 DB에 없을 경우 Fallback Chain을 통해 일반 답변 도구로 자동 전환됩니다.

**예시**: "Attention이 뭐야?" → Easy 모드는 초보자용 쉬운 설명, Hard 모드는 전문가용 상세 설명(Query/Key/Value 벡터, Self-Attention 등) 제공.
...
PostgreSQL + pgvector 기반의 **하이브리드 검색**(벡터 검색 70% + 키워드 검색 30%)으로 로컬 논문 데이터베이스에서 관련 논문 Top-5를 검색하는 도구입니다.

MultiQueryRetriever로 쿼리를 확장하여 검색 품질을 향상시키며, 검색 실패 시 Fallback Chain(web_search → general)을 통해 자동으로 다른 도구로 전환됩니다.

**예시**: "Transformer 논문 설명해줘" → 벡터 유사도 기반으로 관련 논문 5편 검색 후 Easy/Hard 난이도에 맞춰 답변 생성.
...
Tavily Search API를 사용하여 **최신 논문 정보**를 실시간 웹에서 검색하고, arXiv 논문을 자동으로 DB에 저장하는 도구입니다.

"최신", "최근", "2024년" 등 시간 키워드가 포함되면 RAG 검색을 건너뛰고 Web 검색이 최우선 실행되며, RAG 검색 실패 시 Fallback으로도 작동합니다.

**예시**: "2025년 최신 LLM 논문은?" → Tavily API로 웹 검색 후 arXiv 논문 발견 시 자동으로 PostgreSQL에 저장하여 다음번 RAG 검색에서 활용 가능.
...
PostgreSQL + pgvector에서 특정 논문의 모든 청크를 조회하여 **난이도별 구조화된 요약**(Easy: 3줄 요약, Hard: 배경/방법/결과/의의 상세 분석)을 생성하는 도구입니다.

Map-Reduce 방식으로 청크별 요약 후 통합하며, 요약 품질은 LLM-as-a-Judge로 자동 평가합니다.

**예시**: "BERT 논문 요약해줘" → Easy는 핵심만 3줄, Hard는 연구 배경부터 실험 결과, 한계점, 의의까지 구조화된 상세 분석 제공.
...
자연어 질문을 SQL 쿼리로 자동 변환하여 논문 데이터베이스(papers 테이블)의 통계 정보를 제공하는 도구입니다.

LLM이 질문을 분석하여 SELECT 쿼리를 생성하고, 안전하게 실행(Read-only, Prepared Statements)한 후 결과를 난이도별로 해석합니다.

**예시**: "2024년 논문 몇 편?" → LLM이 `SELECT COUNT(*) FROM papers WHERE EXTRACT(YEAR FROM publish_date) = 2024` 생성 및 실행, Easy는 숫자만 간단히, Hard는 통계 분석과 인사이트까지 제공.
...
LLM을 직접 호출하여 일반적인 질문이나 인사, DB/검색이 필요 없는 질문에 답변하는 도구입니다.

다른 모든 도구의 **최종 Fallback**으로 작동하며, 도구 선택 실패 시 자동으로 선택됩니다.

**예시**: "안녕하세요", "AI란 무엇인가요?" 등 → LLM이 직접 답변 생성, 논문 DB나 웹 검색 없이 빠르게 응답.
...
이전 도구의 실행 결과를 로컬 파일 시스템에 Markdown 파일로 영구 저장하는 최종 단계 도구입니다.

**단일 답변 저장**, **전체 대화 저장**, **난이도별 다중 저장**(요약 도구의 4개 수준 답변을 각각 별도 파일로 저장) 등 다양한 저장 모드를 지원하며, 타임스탬프 + 카운터로 고유 파일명을 자동 생성합니다.

ExperimentManager를 통해 세션별 폴더에 저장하여 실험 추적이 용이합니다.

**예시**: "GPT 논문 찾아서 저장해줘" → search_paper 실행 후 결과를 `20251107_153045_response_1.md`로 저장, "Transformer 논문 요약해서 저장해줘" → elementary, beginner 등 난이도별 2~4개 파일로 각각 저장.
...
앱 시작 시 `st.set_page_config()`로 페이지 설정을 구성합니다.

레이아웃과 테마를 설정하며, `st.session_state`로 채팅 세션 목록을 초기화합니다.

사용자 인증 정보와 난이도 설정도 함께 초기화됩니다.

환경 변수 기반 인증 시스템으로 개인 대화를 보호합니다.

전체 너비 레이아웃(wide)과 사이드바 기본 열림 상태로 직관적인 사용자 경험을 제공합니다.

##### 2. 멀티 세션 관리 시스템

ChatGPT 스타일의 세션 관리 시스템입니다.

**UUID 기반 고유 ID 생성**으로 각 세션을 구분합니다.

**날짜별 자동 그룹화**로 세션을 오늘/어제/지난 7일/그 이전으로 정리합니다.

**세션 전환 및 삭제** 기능을 제공합니다.

각 세션은 독립적인 메시지 히스토리와 메타데이터를 보유합니다.

LocalStorage 연동으로 브라우저 새로고침 시에도 세션 목록이 유지됩니다.

##### 3. 채팅 인터페이스 시스템

`st.chat_message()` 컴포넌트로 사용자/AI 메시지를 말풍선 형태로 렌더링합니다.

**도구 배지 색상 코딩**으로 각 도구를 시각적으로 구분합니다(search_paper: 파란색, glossary: 보라색).

**출처 Expander**에서 논문/웹/DB 아이콘으로 출처를 명확히 표시합니다.

**평가 결과 별점 표시**로 답변 품질을 투명하게 공개합니다.

**메시지 복사 기능**(clipboard API)으로 사용자 편의성을 제공합니다.

이러한 기능들로 AI 작동 과정을 투명하게 공개하며 신뢰도를 향상시킵니다.

##### 4. 실시간 스트리밍 시스템

`StreamlitCallbackHandler`가 LangChain 이벤트 시스템과 통합되어 있습니다.

LLM 토큰 생성 시마다 `st.empty()` placeholder에 토큰을 누적합니다.

이를 통해 실시간으로 답변을 출력합니다.

**커서 효과("▌")**로 타이핑 중임을 표시합니다.

**도구 실행 상태 스피너**로 현재 진행 상황을 보여줍니다.

**Agent 액션 로깅**으로 AI의 사고 과정을 투명하게 공개합니다.

도구별 아이콘과 색상 매핑으로 시각적 구분이 가능하며, ChatGPT와 동일한 사용자 경험을 제공합니다.

##### 5. 데이터 영속화 시스템

JavaScript `localStorage` API와 Python 간 브릿지를 구성합니다.

세션 데이터를 JSON 형태로 브라우저에 저장합니다.

`st.session_state`와 자동 동기화되어 데이터 일관성을 유지합니다.

브라우저 새로고침이나 탭 닫기 후에도 대화 내용이 보존됩니다.

Markdown 포맷 내보내기 기능으로 대화 기록을 로컬에 보관할 수 있습니다.
...
![실행-메인 페이지](docs/images/run_img/1_main.png)

<details>
<summary><strong>실행 결과 상세 이미지</strong></summary>

#### 단일요청-'왜 AI 학습에 GPU를 쓰는거야?'-일반 답변
![실행-메인 페이지](docs/images/run_img/2_easy_1.png)

#### 다중요청-'RAG가 뭔지 설명하고 관련 논문도 요약해서 저장해줘'-요약집 검색+논문 검색-논문 요약-저장
![실행-메인 페이지](docs/images/run_img/2_easy_2.png)

#### 처리 과정 보기: 사용된 도구의 처리 과정 확인
![처리 과정 보기-용어집 검색](docs/images/run_img/2_easy_3.png)
![처리 과정 보기-용어집에 검색된 결과를 초보자 모드로 답변 생성](docs/images/run_img/2_easy_4.png)
![처리 과정 보기-논문 검색된 결과를 초보자 모드로 답변 생성](docs/images/run_img/2_easy_5.png)

#### 도구 선택 이유: 도구 선택 방법과 도구 처리 순서 확인
  - 선택 방법
    - keyword_fallback: 사용자 요청/질문의 키워드가 패턴에 없을 경우 키워드 기반 매칭하여 도구 선택
    - pattern_based: 사용자 다중 요청/질문의 키워드들이 미리 지정한 패턴과 매칭될 경우 패턴에 정의된 순서로 도구 선택
    - question_type: 사용자 메세지가 질문인지 파악하여 질문 유형에 가장 적합한 도구 선택
    - llm: LLM이 질문 분석 후 LLM이 도구 선택
  - 파이프라인:
    - 다중 요청/질문일 경우 도구 실행 순서 나열

![도구 선택 이유](docs/images/run_img/2_easy_6.png)

#### 도구 실행 과정: 다중 요청/질문일 경우 사용된 도구 실행 수서 별로 나열
  - RAG 용어집 검색->성공->용어집 테이블에 기재된 용어 설명 사용
  <br>ㄴ>실패->일반 답변
  - RAG 논문 검색->성공->논문 요약
  <br>     ㄴ>실패->Web 논문 검색->성공->논문 요약
  <br>ㄴ>실패->실패->일반 답변->논문 요약
- RAG 논문 검색 성공하여 Web 논문 검색 도구와 일반 답변 도구 실행 생략
- RAG 논문 검색->논문 요약->저장됨

![도구 실행 과정](docs/images/run_img/2_easy_7.png)

#### 초보자 모드-수준 1-초등학생용
![초보자 모드-수준 1-초등학생용](docs/images/run_img/2_easy_8.png)

#### 초보자 모드-수준 2-초급자용
![초보자 모드-수준 2-초급자용](docs/images/run_img/2_easy_9.png)

#### 성능 평가 점수
- 정확도, 관련성, 초보자/전문가 모드에 따른 답변 생성 난이도 적합성, 출처 명시 여부를 LLM이 판단하여 점수 도출
- LLM이 점수 수치를 지정한 근거를 평가 코멘트로 작성
  - LLM은 최근/최신 논문일 경우 실재 존재 여부를 불확실하다고 판단하여 실제로 RAG 논문 테이블에 존재하는 자료임에도 불구하고 낮은 점수를 선정하는 문제가 있음.

![성능 평가 점수](docs/images/run_img/2_easy_10.png)

#### 용어집 페이지
- RAG 용어집, RAG 논문 검색, Web 논문 검색, 일반 답변의 LLM이 생성한 답변에서 IT 용어를 추출하여 AI/ML과 관련성 깊은 순서로 우선순위를 판단하여 사용자가 지정한 용어 추출 수만큼 용어집 테이블에 추가.
- 카테고리는 LLM이 판단하여 지정.
- 용어의 설명은 초보자/전문가 모드를 나눠서 저장
- 사용자가 용어 추출 수를 10개로 지정했을 때 추출된 IT 관련 용어가 3개일 경우 최대 3개만 저장됨.

![용어집 페이지](docs/images/run_img/4_glossary_1.png)

#### 논문 리뷰 챗봇 페이지-왼쪽 패널 용어 추출 설정

![실행-메인 페이지-왼쪽 패널 용어 추출 설정](docs/images/run_img/4_glossary_2.png)

#### 용어집-초보자 모드

![용어집-초보자 모드](docs/images/run_img/4_glossary_easy.png)

#### 용어집-전문가 모드
![용어집-전문가 모드](docs/images/run_img/4_glossary_hard.png)
...
**시스템 설계**:
- [`docs/modularization/`](docs/modularization/) - 전체 시스템 모듈화 문서 (14개 파일)
- [`docs/PRD/`](docs/PRD/) - 제품 요구사항 정의 문서 (16개 파일)
...
- [`docs/scenarios/`](docs/scenarios/) - 도구별 상세 시나리오 (7개 파일)

**Q&A 문서**:
- [`docs/QnA/`](docs/QnA/) - 시스템별 및 도구별 Q&A 문서 (8개 파일)

**아키텍처 문서**:
- [`docs/architecture/single_request/`](docs/architecture/single_request/) - 단일 요청 아키텍처 (7개 파일)
...
- [`docs/issues/`](docs/issues/) - 구현 이슈 및 개선사항 문서 (28개 파일)
  ...
  | [`01-9_ExperimentManager_도구별_로그_기록_미구현.md`](docs/issues/01-9_ExperimentManager_도구별_로그_기록_미구현.md) | 실험 폴더 자동 관리 시스템에 일부 도구 로그 파일 미생성 문제 해결 |
