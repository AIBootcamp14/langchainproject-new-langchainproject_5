...
- **Agent 라우팅 단계**: 라우터 노드가 질문 유형을 분석하여 7개 도구 중 적절한 도구 선택
...
> **참고**: 성능 평가는 답변 생성 후 실시간으로 자동 실행되며, Streamlit UI는 평가 완료 후 자동으로 입력창으로 복귀하여 연속 질문이 가능합니다.
...
graph TB
    ...
    subgraph Business_Layer["🔹 비즈니스 계층"]
        direction LR
        Agent[agent/graph.py] --> Tools[tools/*]
