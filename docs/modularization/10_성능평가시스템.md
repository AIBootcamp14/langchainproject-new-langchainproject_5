...
**아키텍처 흐름 설명:**
- 입력 단계에서 사용자 질문, AI 답변, 참고 문서, 난이도 모드를 받습니다
- 평가 단계에서 AnswerEvaluator가 GPT-4를 호출하여 평가 프롬프트를 포맷팅하고, JSON 형식으로 평가 결과를 파싱합니다
- 저장 단계에서 평가 결과를 PostgreSQL evaluation_results 테이블에 저장하고, 평가 통계를 집계합니다
...
| 함수 | 설명 | 반환 타입 |
|------|------|----------|
| `create_evaluation_table()` | evaluation_results 테이블 생성 | None |
