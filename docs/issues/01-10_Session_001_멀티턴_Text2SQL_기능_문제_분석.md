# Session 001 실험 분석: 멀티턴 대화 및 Text2SQL 기능 문제
...
| 작업 | 예상 시간 | 담당자 | 파일 |
|-----|---------|--------|------|
| 1-1. 맥락 참조 키워드 확장 | 10분 | @최현화 | `src/agent/nodes.py:51` |
| 1-2. 키워드 확장 테스트 | 15분 | @최현화 | 테스트 스크립트 실행 |
| 1-3. 질문 재작성 프롬프트 개선 | 20분 | @최현화 | `src/agent/nodes.py` (LLM 라우팅 프롬프트) |
| 1-4. 멀티턴 통합 테스트 | 30분 | @최현화 | 초보자 질문 리스트 재실행 |
...
| 작업 | 예상 시간 | 담당자 | 파일 |
|-----|---------|--------|------|
| 2-1. 파이프라인 조기 종료 로직 추가 | 30분 | @최현화 | `src/agent/nodes.py` |
| 2-2. 실패 감지 정규식 수정 | 15분 | @최현화 | `src/agent/nodes.py` |
| 2-3. text2sql 응답 형식 표준화 | 20분 | @최현화 | `src/tools/text2sql.py` |
| 2-4. 파이프라인 패턴 단순화 | 25분 | @최현화 | `configs/multi_request_patterns.yaml` |
| 2-5. text2sql 통합 테스트 | 30분 | @최현화 | 초보자 질문 리스트 재실행 |
...
| 작업 | 예상 시간 | 담당자 | 파일 |
|-----|---------|--------|------|
| 3-1. Fallback 종료 플래그 추가 | 20분 | @최현화 | `src/agent/fallback.py` (추정) |
| 3-2. 파이프라인에서 종료 플래그 확인 | 15분 | @최현화 | `src/agent/nodes.py` |
| 3-3. Fallback 통합 테스트 | 25분 | @최현화 | 전문가 질문 리스트 실행 |
...
| 작업 | 예상 시간 | 담당자 | 파일 |
|-----|---------|--------|------|
| 4-1. 용어 추출 JSON 파싱 오류 수정 | 30분 | @최현화 | `src/tools/glossary.py` (추정) |
| 4-2. 대화 히스토리 윈도우 확대 | 40분 | @최현화 | `src/agent/nodes.py` |
