...
graph TB
    ...
    subgraph DataLayer["🔺 데이터 & LLM 레이어"]
        ...
        LLM[🤖 OpenAI GPT-4 / Solar<br/>답변 생성]
    end
...
| 구성 요소 | Streamlit 함수 | 설정 | 설명 |
|----------|---------------|------|------|
| 난이도 선택 | st.selectbox | 옵션: ["Easy 모드 (초심자용)", "Hard 모드 (전문가용)"] | 사용자 난이도 선택 |
| 세션 상태 | st.session_state | messages 리스트 초기화 | 채팅 히스토리 저장 |
| 메시지 표시 | st.chat_message | role 기반 메시지 컨테이너 | 기존 메시지 표시 |
| 사용자 입력 | st.chat_input | 플레이스홀더: "논문에 대해 질문해주세요" | 사용자 질문 입력 |

**Agent 호출 파라미터:**

| 파라미터 | 값 | 설명 |
|---------|-----|------|
| question | 사용자 입력 (prompt) | 질문 내용 |
| difficulty | "easy" 또는 "hard" | 선택한 난이도에 따라 결정 |
...
**필요 라이브러리:** `langgraph.graph.StateGraph`, `langgraph.graph.END`, `typing.TypedDict`
...
- 질문을 분석하여 적절한 도구 선택
- LLM에게 라우팅 프롬프트 전달
- 도구 옵션: search_paper, web_search, glossary, summarize, save_file, general
- tool_choice를 state에 저장하고 반환
...
| 단계 | 동작 | 설명 |
|------|------|------|
| 1 | StateGraph(AgentState) 생성 | 상태 그래프 초기화 |
| 2 | 노드 추가 | router, search_paper, web_search, glossary, summarize, save_file, general |
| 3 | 진입점 설정 | set_entry_point("router") |
| 4 | 조건부 엣지 추가 | router에서 tool_choice에 따라 각 도구 노드로 분기 |
| 5 | 종료 엣지 추가 | 모든 도구 노드에서 END로 연결 |
| 6 | 컴파일 | workflow.compile()로 실행 가능한 그래프 생성 |
...
| 단계 | 동작 | 설명 |
|------|------|------|
| 1 | Vector DB 유사도 검색 | vectorstore.similarity_search(query, k=5) |
| 2 | 문서 순회 | 검색된 각 문서에서 paper_id 추출 |
| 3 | 메타데이터 조회 | PostgreSQL에서 get_paper_metadata(paper_id) 호출 |
| 4 | 결과 구성 | title, authors, year, content를 딕셔너리로 저장 |
| 5 | 반환 | format_search_results(results)로 포맷하여 반환 |
...
| include_answer | True | 검색 결과에 요약 답변 포함 |
...
**반환값:** str - 용어 설명 또는 RAG 검색 결과

**처리 흐름:**

| 단계 | 동작 | 설명 |
|------|------|------|
| 1 | PostgreSQL 쿼리 | glossary 테이블에서 term ILIKE 검색 |
| 2 | 결과 확인 | 용어 발견 시 difficulty에 따라 설명 선택 |
| 3 | 설명 반환 | easy: easy_explanation, hard: hard_explanation |
| 4 | 대체 검색 | 용어 미발견 시 search_paper_database("{term} 정의") 호출 |
...
graph TB
    ...
    subgraph Retrieval["🔹 문서 검색 & 재순위화"]
        direction LR
        VDB[(💾 Vector DB<br/>pgvector)] --> TopK[Top-K 문서<br/>상위 5개]
...
| 파라미터 | 타입 | 설명 |
|---------|------|------|
| vectorstore | PGVector | Vector DB 인스턴스 |
| embeddings | Embeddings | 임베딩 모델 인스턴스 |

**메서드:**

| 메서드 | 파라미터 | 반환값 | 설명 |
|--------|----------|--------|------|
| retrieve | query: str, k: int = 5 | List[Document] | 유사도 기반 검색 |
| retrieve_with_rerank | query: str, k: int = 5 | List[Document] | Reranking을 포함한 검색 |

**retrieve 메서드 처리 흐름:**

| 단계 | 동작 |
|------|------|
| 1 | vectorstore.similarity_search(query, k=k) 호출 |
| 2 | 검색된 문서 리스트 반환 |

**retrieve_with_rerank 메서드 처리 흐름:**

| 단계 | 동작 |
|------|------|
| 1 | vectorstore.similarity_search(query, k=k*2)로 후보 조회 |
| 2 | self.rerank(query, docs)로 재정렬 수행 |
| 3 | 상위 k개 문서만 반환 (reranked_docs[:k]) |
...
**EASY_MODE_PROMPT (초심자용):**

| 구성 요소 | 내용 |
|----------|------|
| 역할 | AI 초심자를 위한 논문 리뷰 어시스턴트 |
| 답변 규칙 | 1. 전문 용어는 쉬운 말로 풀어서 설명<br/>2. 실생활 비유 사용<br/>3. 수식은 최소화<br/>4. 핵심 아이디어 3가지 이내로 요약 |
| 프롬프트 변수 | {context}: RAG 검색 결과 컨텍스트<br/>{question}: 사용자 질문 |

**HARD_MODE_PROMPT (전문가용):**

| 구성 요소 | 내용 |
|----------|------|
| 역할 | AI 전문가를 위한 논문 리뷰 어시스턴트 |
| 답변 규칙 | 1. 기술적 세부사항 포함<br/>2. 수식 및 알고리즘 설명<br/>3. 관련 논문 비교<br/>4. 구현 세부사항 |
| 프롬프트 변수 | {context}: RAG 검색 결과 컨텍스트<br/>{question}: 사용자 질문 |
...
[사용자] --> [Streamlit Cloud]
    ...
    ┌──────────────┼──────────────┼──────────────┐
    ↓              ↓              ↓              ↓
[PostgreSQL]  [OpenAI API]  [Solar API]  [Tavily API]
 +pgvector     (클라우드)     (클라우드)    (클라우드)
 (Docker)
```
...
**필요 라이브러리:** `functools.lru_cache`

**캐싱 함수:**

| 함수명 | 데코레이터 설정 | 설명 |
|--------|----------------|------|
| cached_llm_call | @lru_cache(maxsize=100) | LLM 응답 캐싱 (최대 100개) |
| cached_similarity_search | @lru_cache(maxsize=50) | Vector DB 검색 결과 캐싱 (최대 50개) |

**함수 상세:**

| 함수명 | 파라미터 | 반환값 | 설명 |
|--------|----------|--------|------|
| cached_llm_call | prompt_hash | LLM 응답 | 프롬프트 해시로 LLM 호출 결과 캐싱 |
| cached_similarity_search | query_hash, k | List[Document] | 쿼리 해시로 유사도 검색 결과 캐싱 |

### 8.2 비동기 처리

**필요 라이브러리:** `asyncio`

**함수: async_agent_invoke**
...
| 단계 | 동작 |
|------|------|
| 1 | agent_executor.ainvoke() 비동기 호출 |
| 2 | question, difficulty를 딕셔너리로 전달 |
| 3 | 결과 반환 |
