...
graph TB
    subgraph MainFlow["📋 프로젝트 전체 구조"]
        ...
        subgraph Configs["🔸 설정 (configs/)"]
            direction LR
            DBConfig[db_config.yaml<br/>DB 설정] --> ModelConfig[model_config.yaml<br/>모델 설정]
        ...
        subgraph Prompts["🔺 프롬프트 엔지니어링 (prompts/)"]
            direction LR
            RoutingPrompts[routing_prompts.json<br/>라우팅 프롬프트] --> ToolPrompts[tool_prompts.json<br/>도구 프롬프트]
    ...
    linkStyle 32,33,34,35 stroke:#616161,stroke-width:3px
    linkStyle 36,37,38,39 stroke:#616161,stroke-width:3px
```
