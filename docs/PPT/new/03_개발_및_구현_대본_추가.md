...
```mermaid
graph TB
    subgraph MainFlow["📋 도구 자동 전환 Fallback Chain"]
        direction TB

        subgraph Execution["🔸 도구 실행"]
            direction LR
            A([도구 실행 시작]) --> B[선택된 도구 실행]
            B --> C{실행 결과<br/>확인}
        end

        subgraph FallbackCheck["🔹 Fallback 판단"]
            direction LR
            C -->|성공| D[답변 반환]
            C -->|실패| E{재시도 횟수<br/>확인}
            E -->|< max_retries| F[Fallback Router]
            E -->|>= max_retries| G[General 도구<br/>강제 실행]
        end

        subgraph FallbackExec["🔺 대체 도구 실행"]
            direction LR
            F --> H[우선순위 목록<br/>다음 도구 선택]
            H --> I[대체 도구 실행]
            I --> J{실행 결과<br/>확인}
            J -->|성공| K[답변 반환]
            J -->|실패| E
    ...
    %% Subgraph 스타일
    style Execution fill:#e0f7fa,stroke:#006064,stroke-width:3px,color:#000
    style FallbackCheck fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style FallbackExec fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Final fill:#ffebee,stroke:#c62828,stroke-width:3px,color:#000
...
Fallback 체인은 최대 3회까지 재시도하며,
재시도 횟수를 초과하면 최종적으로 일반 답변 도구가
LLM의 자체 지식으로 답변을 생성합니다.
...
```mermaid
graph TB
    subgraph MainFlow["📋 RAG 논문 검색 3단계 파이프라인"]
        direction TB

        subgraph Indexing["🔸 1. Indexing (사전 준비)"]
            direction LR
            A([논문 PDF]) --> B[PyPDFLoader<br/>로드]
            B --> C[RecursiveCharacterTextSplitter<br/>청킹]
            C --> D[OpenAI<br/>Embedding]
            D --> E[💾 PGVector<br/>저장]
        end

        subgraph Retrieval["🔹 2. Retrieval (검색)"]
            direction LR
            F[사용자 질문] --> G[OpenAI<br/>Embedding]
            G --> H[PGVector<br/>유사도 검색]
            H --> I[Top-5<br/>청크 추출]
            I --> J[paper_id<br/>추출]
            J --> K[💾 PostgreSQL<br/>메타데이터 조회]
        end

        subgraph Generation["🔺 3. Generation (생성)"]
            direction LR
            L[검색 결과 +<br/>메타데이터] --> M{난이도<br/>선택}
            M -->|Easy| N[Solar Pro2<br/>쉬운 설명]
            M -->|Hard| O[GPT-4<br/>기술 상세]
            N --> P[최종 답변<br/>+ 출처]
            O --> P
            P --> Q([✅ 사용자에게<br/>전달])
        end
    ...
    %% Subgraph 스타일
    style Indexing fill:#e0f7fa,stroke:#006064,stroke-width:3px,color:#000
    ...
    %% 노드 스타일 (Indexing - 청록 계열)
    style A fill:#4db6ac,stroke:#00695c,stroke-width:3px,color:#000
    style B fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000
    style C fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000
    style D fill:#4dd0e1,stroke:#006064,stroke-width:2px,color:#000
    style E fill:#26c6da,stroke:#00838f,stroke-width:2px,color:#000

    %% 노드 스타일 (Retrieval - 보라 계열)
    ...
    style K fill:#ba68c8,stroke:#6a1b9a,stroke-width:2px,color:#fff

    %% 노드 스타일 (Generation - 녹색 계열)
    style L fill:#81c784,stroke:#2e7d32,stroke-width:2px,color:#000
    style M fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#000
    style N fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style O fill:#a5d6a7,stroke:#388e3c,stroke-width:2px,color:#000
    style P fill:#66bb6a,stroke:#1b5e20,stroke-width:2px,color:#000
    style Q fill:#66bb6a,stroke:#2e7d32,stroke-width:3px,color:#000
```
...
```mermaid
graph TB
    subgraph MainFlow["📋 용어집 하이브리드 검색 (3단계 Fallback)"]
        direction TB

        subgraph Stage1["🔸 1차 검색: PostgreSQL 직접 매칭"]
            direction LR
            A([사용자 질문<br/>'BERT가 뭐야?']) --> B[용어 추출]
            B --> C[💾 glossary 테이블<br/>ILIKE 검색]
            C --> D{정확 매칭<br/>발견?}
            D -->|Yes| E{난이도<br/>선택}
            E -->|Easy| F[easy_explanation<br/>반환]
            E -->|Hard| G[hard_explanation<br/>반환]
        end

        subgraph Stage2["🔹 2차 검색: Vector 유사도 검색"]
            direction LR
            D -->|No| H[질문 임베딩]
            H --> I[💾 glossary_embeddings<br/>벡터 검색]
            I --> J{유사 용어<br/>발견?}
            J -->|Yes| K[유사 용어<br/>정의 반환]
            J -->|No| L[3차 검색으로]
        end

        subgraph Stage3["🔺 3차 검색: RAG 논문 검색"]
            direction LR
            L --> M[RAG 논문 검색<br/>도구 호출]
            M --> N[💾 PGVector<br/>논문 본문 검색]
            N --> O[용어 정의<br/>문맥 추출]
            O --> P[LLM 요약<br/>+ 설명]
        end

        subgraph Final["💡 최종 답변"]
            direction LR
            F --> Q([✅ 답변 반환])
            G --> Q
            K --> Q
    ...
    style Stage2 fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style Stage3 fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Final fill:#ffebee,stroke:#c62828,stroke-width:3px,color:#000
    ...
    %% 노드 스타일 (2차 검색 - 보라 계열)
    style H fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style I fill:#ba68c8,stroke:#6a1b9a,stroke-width:2px,color:#fff
    style J fill:#ce93d8,stroke:#7b1fa2,stroke-width:2px,color:#000
...
```mermaid
graph TB
    subgraph MainFlow["📋 LLM-as-a-Judge 성능 평가 시스템"]
        direction TB

        subgraph Input["🔸 평가 입력"]
            direction LR
            A([답변 생성 완료]) --> B[질문 + 답변<br/>+ 참고 문서]
            B --> C[난이도 정보<br/>(Easy/Hard)]
            C --> D[선택된 도구<br/>정보]
        end

        subgraph Evaluation["🔹 6가지 기준 평가"]
            direction LR
            D --> E[GPT-4 평가자<br/>LLM 호출]
            E --> F1[정확도<br/>(0-10점)]
            E --> F2[관련성<br/>(0-10점)]
            E --> F3[난이도 적합성<br/>(0-10점)]
            E --> F4[출처 명시<br/>(0-10점)]
            E --> F5[도구 호출<br/>(0-10점)]
            E --> F6[가독성<br/>(0-10점)]
        end

        subgraph Scoring["🔺 점수 집계"]
            direction LR
            F1 --> G[총점 계산<br/>(60점 만점)]
            F2 --> G
            F3 --> G
            F4 --> G
            F5 --> G
        ...
        subgraph Storage["💡 결과 저장 & 활용"]
            direction LR
            I --> K[💾 PostgreSQL<br/>evaluation_results]
            J --> K
            K --> L[평가 리포트<br/>생성]
            L --> M[UI 표시]
            L --> N[시스템 개선<br/>피드백]
    ...
    style Evaluation fill:#f3e5f5,stroke:#4a148c,stroke-width:3px,color:#000
    style Scoring fill:#e8f5e9,stroke:#1b5e20,stroke-width:3px,color:#000
    style Storage fill:#ffebee,stroke:#c62828,stroke-width:3px,color:#000
    ...
    style F1 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style F2 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style F3 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style F4 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style F5 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    style F6 fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000
    ...
    style N fill:#e57373,stroke:#c62828,stroke-width:2px,color:#000
```
