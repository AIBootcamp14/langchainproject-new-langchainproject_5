...
```
prompts/
├── routing_prompts.json                    # ✅ 현재 사용 (라우팅 프롬프트)
├── tool_prompts.json                       # ✅ 현재 사용 (도구별 프롬프트)
├── evaluation_prompts.json                 # ✅ 현재 사용 (평가 프롬프트)
├── question_generation_prompts.json        # ✅ 현재 사용 (질문 생성)
├── golden_dataset.json                     # ✅ 현재 사용 (테스트 데이터셋)
├── evaluation_prompts_v.01.json            # ⚠️ 이전 버전 (사용 안함)
├── golden_dataset_v01.json                 # ⚠️ 이전 버전 (사용 안함)
├── question_generation_prompts copy.json   # ⚠️ 백업 파일 (사용 안함)
└── tool_prompts_v2.json                    # ⚠️ 빈 파일 (사용 안함)
```
...
#### ⚠️ **사용되지 않는 파일 (4개)** - 향후 정리 필요

| 파일명 | 비고 |
|--------|------|
| `evaluation_prompts_v.01.json` | 이전 버전 (10,588 bytes) |
| `golden_dataset_v01.json` | 이전 버전 (23,274 bytes) |
| `question_generation_prompts copy.json` | 백업 파일 (10,842 bytes) |
| `tool_prompts_v2.json` | 빈 파일 (0 bytes) |
...
graph TB
    ...
    subgraph "2️⃣ JSON 프롬프트 파일"
        RP[routing_prompts.json<br/>라우팅 프롬프트]
        TP[tool_prompts.json<br/>도구별 프롬프트]
    end

    subgraph "3️⃣ 프롬프트 로더"
        Loader[src/prompts/loader.py]
        Init[src/prompts/__init__.py]
    end

    subgraph "4️⃣ Agent 그래프"
        Router[Router Node<br/>도구 선택]

        subgraph "도구 노드 (7개)"
            General[General Answer<br/>✅ JSON 적용]
            WebSearch[Web Search<br/>✅ JSON 적용]
            Summarize[Summarize<br/>✅ JSON 적용]
            SearchPaper[Search Paper<br/>❌ 미적용]
            Glossary[Glossary<br/>❌ 미적용]
            SaveFile[Save File<br/>❌ 미적용]
            Text2SQL[Text-to-SQL<br/>🆕 신규 추가]
        end
    end

    subgraph "5️⃣ LLM 호출"
        LLM[LLM Client<br/>난이도별 모델 선택]
    end

    subgraph "6️⃣ 응답 생성"
        Response[최종 답변<br/>난이도별 스타일]
    end
    ...
    classDef jsonFile fill:#FFE6CC,stroke:#D79B00,stroke-width:2px
    classDef loader fill:#DAE8FC,stroke:#6C8EBF,stroke-width:2px
    classDef applied fill:#D5E8D4,stroke:#82B366,stroke-width:2px
    classDef notApplied fill:#F8CECC,stroke:#B85450,stroke-width:2px
    classDef newNode fill:#E1D5E7,stroke:#9673A6,stroke-width:2px

    class RP,TP jsonFile
    class Loader,Init loader
    class General,WebSearch,Summarize applied
    class SearchPaper,Glossary,SaveFile notApplied
    class Text2SQL newNode
```
...
sequenceDiagram
    ...
    Note over Node,LLM: 예시: general_answer_node()

    Node->>Init: get_tool_prompt("general_answer", "easy")
    Init->>Loader: load_tool_prompts()
    Loader->>JSON: 파일 읽기 (tool_prompts.json)
    JSON-->>Loader: JSON 데이터 반환
    Loader-->>Init: 파싱된 딕셔너리
    Init-->>Node: system_prompt 텍스트

    Node->>LLM: SystemMessage(content=system_prompt)
    Node->>LLM: HumanMessage(content=question)
    LLM-->>Node: 답변 생성
    Node-->>Node: state["final_answer"] = 답변
```
...
graph LR
    ...
    subgraph "LLM Client 초기화"
        LLMClient[LLMClient.from_difficulty]
    end

    subgraph "모델 선택"
        EasyModel[gpt-4o-mini<br/>빠르고 저렴]
        HardModel[gpt-4o<br/>고성능 정확도]
    end

    subgraph "프롬프트 로드"
        EasyPrompt[Easy 프롬프트<br/>쉬운 언어, 비유]
        HardPrompt[Hard 프롬프트<br/>전문 용어, 수식]
    end
    ...
    classDef easy fill:#D5E8D4,stroke:#82B366
    classDef hard fill:#FFE6CC,stroke:#D79B00

    class EasyModel,EasyPrompt easy
    class HardModel,HardPrompt hard
```
...
### ❌ JSON 프롬프트 미적용 (3개 노드) - 향후 작업

#### 5. Glossary Node (용어집 노드)

**파일**: `src/tools/glossary.py`
**현재 상태**: 하드코딩된 프롬프트 사용 (추정)
**향후 작업**: `tool_prompts.json` → `glossary_prompts` 적용 필요
...
#### 6. Search Paper Node (논문 검색 노드)

**파일**: `src/tools/search_paper.py`
**현재 상태**: 하드코딩된 프롬프트 사용 (추정)
**향후 작업**: `tool_prompts.json` → `search_paper_prompts` 적용 필요
...
| **JSON 프롬프트 적용 노드** | 0개 | 4개 (router, general, web_search, summarize) | 프롬프트 엔지니어링 적용 |
...
### 1. 프롬프트 적용 확대 (우선순위: 높음)

#### 미적용 노드에 JSON 프롬프트 적용

| 노드 | 현재 상태 | 작업 내용 | 예상 난이도 |
|------|-----------|-----------|-------------|
| **Glossary Node** | ❌ 미적용 | `get_tool_prompt("glossary", difficulty)` 적용 | ⭐ 쉬움 |
| **Search Paper Node** | ❌ 미적용 | `get_tool_prompt("search_paper", difficulty)` 적용 | ⭐ 쉬움 |
| **Save File Node** | ❌ 미적용 | 메시지 템플릿 적용 (선택 사항) | ⭐ 쉬움 |
...
# 2. glossary_node() 함수 내부
difficulty = state.get("difficulty", "easy")
system_content = get_tool_prompt("glossary", difficulty)  # JSON 로드
system_msg = SystemMessage(content=system_content)
...
### 2. Golden Dataset 확장 (우선순위: 중간)

#### text2sql 예제 추가

**현재 상태**: Golden Dataset에 text2sql 예제 없음
**작업 내용**: `golden_dataset.json`에 5개 예제 추가
...
```json
{
  ...
  "expected_answer_keywords": ["2024", "개수", "통계"],
  "category": "통계 조회",
  "complexity": "medium"
},
{
  "question": "카테고리별 논문 수를 보여줘",
  "difficulty": "hard",
  "expected_tool": "text2sql",
  "expected_answer_keywords": ["카테고리", "분포", "통계"],
  "category": "통계 조회",
  "complexity": "high"
},
{
  ...
  "difficulty": "hard",
  "expected_tool": "text2sql",
  "expected_answer_keywords": ["Top", "순위", "인용"],
  "category": "통계 조회",
  "complexity": "high"
}
```
...
### 3. 이전 버전 파일 정리 (우선순위: 낮음)

#### prompts 폴더 정리

**작업 내용**: 사용하지 않는 파일 4개 삭제 또는 백업 폴더 이동

**삭제 대상**:
- `evaluation_prompts_v.01.json`
- `golden_dataset_v01.json`
- `question_generation_prompts copy.json`
- `tool_prompts_v2.json`

**정리 방법**:
```bash
# 옵션 1: 백업 폴더로 이동
mkdir prompts/backup
mv prompts/*_v*.json prompts/backup/
mv prompts/*copy*.json prompts/backup/

# 옵션 2: 삭제 (Git 이력 보존됨)
git rm prompts/evaluation_prompts_v.01.json
git rm prompts/golden_dataset_v01.json
git rm prompts/question_generation_prompts\ copy.json
git rm prompts/tool_prompts_v2.json
```
