...
```mermaid
graph TB
    subgraph MainFlow["📋 멀티턴 대화 상세 실행 흐름"]
        direction TB

        subgraph SessionInit["🔸 세션 초기화"]
            direction LR
            A[ui/app.py<br/>initialize_chat_sessions] --> B[session_state<br/>초기화]
            B --> C[chats 딕셔너리<br/>생성]
            C --> D[current_chat_id<br/>None]
        end

        subgraph FirstTurnInit["🔹 첫 번째 턴 시작"]
            direction LR
            E[사용자<br/>첫 질문 입력] --> F[create_new_chat<br/>호출]
            F --> G[UUID 생성<br/>chat_id]
            G --> H[세션 데이터<br/>생성]
            H --> I[messages 빈<br/>리스트 초기화]
        end

        subgraph FirstTurnRouting["🔺 첫 턴 라우팅"]
            direction LR
            J[router_node<br/>실행] --> K[messages<br/>리스트 확인]
            K --> L{messages<br/>비어있음?}
            L -->|Yes| M[일반 라우팅<br/>패턴 매칭 시도]
            L -->|No| N[멀티턴 체크]
        end

        subgraph FirstTurnTool["🔶 첫 턴 도구 실행"]
            direction LR
            O[도구 선택<br/>완료] --> P[도구 노드<br/>실행]
            P --> Q[답변 생성<br/>final_answer]
            Q --> R[source_documents<br/>저장]
        end

        subgraph FirstTurnSave["💾 첫 턴 저장"]
            direction LR
            S[add_message_to_current_chat<br/>user] --> T[messages에<br/>질문 추가]
            T --> U[add_message_to_current_chat<br/>assistant] --> V[messages에<br/>답변 추가]
            V --> W[채팅 제목<br/>업데이트]
        end

        subgraph SecondTurnStart["💡 두 번째 턴 시작"]
            direction LR
            X[사용자<br/>맥락 참조 질문] --> Y[get_current_messages<br/>호출]
            Y --> Z[이전 messages<br/>로드]
            Z --> AA[state에<br/>messages 전달]
        end

        subgraph ContextDetection["🔷 맥락 참조 감지"]
            direction LR
            AB[router_node<br/>실행] --> AC[contextual_keywords<br/>체크]
            AC --> AD{맥락 키워드<br/>포함?}
            AD -->|Yes| AE[has_contextual_ref<br/>= True]
            AD -->|No| AF[일반 처리]
            AE --> AG{messages<br/>개수 > 1?}
            AG -->|Yes| AH{다중 요청<br/>키워드?}
            AG -->|No| AF
        end

        subgraph MultiTurnMode["✨ 멀티턴 모드"]
            direction LR
            AI[skip_pattern_matching<br/>= True] --> AJ[패턴 매칭<br/>건너뛰기]
            AJ --> AK[LLM 라우팅<br/>직행]
            AK --> AL[routing_prompt<br/>로드]
            AL --> AM[messages 포함<br/>프롬프트 구성]
        end

        subgraph LLMRouting["🔹 LLM 라우팅"]
            direction LR
            AN[LLM에<br/>질문 전달] --> AO[LLM이<br/>messages 분석]
            AO --> AP[컨텍스트<br/>이해]
            AP --> AQ[이전 대화에서<br/>참조 대상 파악]
            AQ --> AR[적절한 도구<br/>선택]
        end

        subgraph SecondTurnTool["🔶 두 번째 턴 도구 실행"]
            direction LR
            AS[선택된 도구<br/>실행] --> AT[이전 답변<br/>참조 가능]
            AT --> AU[새 답변<br/>생성]
            AU --> AV[컨텍스트 일관성<br/>유지]
        end

        subgraph SecondTurnSave["✅ 두 번째 턴 저장"]
            direction LR
            AW[messages에<br/>새 질문 추가] --> AX[messages에<br/>새 답변 추가]
            AX --> AY[세션 상태<br/>업데이트]
            AY --> AZ[UI 리렌더링]
        end

        subgraph SessionManagement["⚠️ 세션 관리 기능"]
            direction LR
            BA[switch_chat<br/>채팅 전환] --> BB[delete_chat<br/>채팅 삭제]
            BB --> BC[export_chat<br/>대화 내보내기]
            BC --> BD[get_chat_list<br/>채팅 목록]
        end
        ...
        FirstTurnInit -.세션 생성.-> SessionManagement
        SecondTurnSave -.세션 유지.-> SessionManagement
    end
    ...
    %% 연결선 스타일 (세션 초기화 - 청록 0~2)
    linkStyle 0 stroke:#006064,stroke-width:2px
    linkStyle 1 stroke:#006064,stroke-width:2px
    linkStyle 2 stroke:#006064,stroke-width:2px

    %% 연결선 스타일 (첫 턴 초기화 - 파랑 3~6)
    linkStyle 3 stroke:#1976d2,stroke-width:2px
    linkStyle 4 stroke:#1976d2,stroke-width:2px
    linkStyle 5 stroke:#1976d2,stroke-width:2px
    linkStyle 6 stroke:#1976d2,stroke-width:2px

    %% 연결선 스타일 (첫 턴 라우팅 - 보라 7~10)
    ...
    linkStyle 54 stroke:#616161,stroke-width:2px,stroke-dasharray:5
