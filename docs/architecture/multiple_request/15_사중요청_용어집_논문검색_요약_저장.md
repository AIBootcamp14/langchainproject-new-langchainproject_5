...
```mermaid
graph TB
    subgraph MainFlow["📋 용어집 → 논문 검색 → 요약 → 저장 파이프라인"]
        direction TB

        subgraph Init["🔸 초기화 & 라우팅"]
            direction LR
            Start([▶️ 시작]) --> A[사용자 질문:<br/>RAG가 뭔지 설명하고<br/>관련 논문도 요약해서 저장해줘]
            A --> B[router_node<br/>패턴 매칭]
            B --> C[Pipeline 설정<br/>4단계 파이프라인]
        end

        subgraph Step1["🔹 1단계: 용어 정의"]
            direction LR
            D[glossary 실행<br/>용어집 검색] --> E{검색 성공?<br/>score >= 0.7}
            E -->|Yes| F[용어 정의 획득<br/>💾 term_definition]
            E -->|No| G[1-F: general<br/>LLM 지식 기반]
            G --> H[용어 설명 생성<br/>💾 term_definition]
        end

        subgraph Step2["🔺 2단계: 논문 검색"]
            direction LR
            I[search_paper 실행<br/>RAG 논문 검색] --> J{검색 성공?<br/>score >= 0.7}
            J -->|Yes| K[논문 정보 획득<br/>💾 papers]
            J -->|No| L[2-F1: web_search<br/>웹 논문 검색]
            L --> M{검색 성공?<br/>결과 존재}
            M -->|Yes| N[웹 논문 획득<br/>💾 papers]
            M -->|No| O[2-F2: general<br/>LLM 지식 기반]
            O --> P[논문 설명 생성<br/>💾 papers]
        end

        subgraph Step3["🔶 3단계: 논문 요약"]
            direction LR
            Q[summarize 실행<br/>논문 요약] --> R{요약 성공?}
            R -->|Yes| S[논문 요약 생성<br/>💾 paper_summary]
            R -->|No| T[3-F: general<br/>LLM 지식 기반]
            T --> U[요약 생성<br/>💾 paper_summary]
        end

        subgraph Step4["✨ 4단계: 파일 저장"]
            direction LR
            V[save_file 실행<br/>세션 데이터 로드] --> W[파일 내용 구성<br/>term_definition +<br/>paper_summary]
            W --> X{저장 성공?}
            X -->|Yes| Y[4개 파일 생성<br/>easy/hard/auto/raw]
            X -->|No| Z[저장 실패<br/>오류 메시지]
        end

        subgraph Output["💡 5단계: 최종 출력"]
            direction LR
            AA[UI 표시] --> AB[저장 완료 메시지<br/>파일 경로 포함]
            AB --> End([✅ 완료])
        end
...
graph TB
    subgraph MainFlow["📋 상세 실행 흐름"]
        ...
        subgraph Step1["🔹 1단계: 용어 정의"]
            ...
            E3 -->|success| E6[세션 저장<br/>💾 term_definition]
        ...
        subgraph Step2["🔺 2단계: 논문 검색"]
            ...
            F3 -->|success| F9[papers 데이터<br/>3단계로 전달]
        ...
        subgraph Step3["🔶 3단계: 논문 요약"]
            ...
            G3 -->|success| G6[세션 저장<br/>💾 paper_summary]
        ...
        subgraph Step4["✨ 4단계: 파일 저장"]
            direction LR
            H1[src/agent/nodes.py<br/>save_file_node] --> H3[세션에서 데이터 가져오기<br/>term_definition +<br/>paper_summary]
            H3 --> H2[src/tools/save_file.py<br/>save_file]
            H2 --> H4{저장 성공?}
            H4 -->|success| H5[4개 파일 생성]
            H4 -->|failed| H6[오류 메시지]
        end

        subgraph Output["💡 응답 반환"]
            direction LR
            I1[최종 응답 생성] --> I2[사용자에게 반환]
        end
