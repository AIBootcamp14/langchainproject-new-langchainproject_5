...
graph TB
    ...
    subgraph Backend["🔹 백엔드"]
        ...
        LC --> LLM1[OpenAI API<br/>GPT-5]
        LC --> LLM2[Solar API<br/>Solar Pro2]
...
- 백엔드 계층에서는 LangGraph와 Langchain을 기반으로 OpenAI GPT-5, Solar API, Tavily Search API를 통합하여 AI Agent 로직을 처리
...
| 구분 | 선택 기술 | 버전 | 이유 |
|------|----------|------|------|
| **LLM API** | OpenAI (GPT-5) + Solar (Solar Pro2) | Latest | OpenAI: 높은 성능, 안정적 Function Calling<br/>Solar: 한국어 특화, 비용 효율적 |
...
**구현 파일**: `src/utils/db.py`

**사용 라이브러리**: `psycopg2.pool.SimpleConnectionPool`
...
| maxconn | 10 | 최대 연결 수 |
| host | POSTGRES_HOST (환경 변수) | PostgreSQL 호스트 |
| port | POSTGRES_PORT (환경 변수) | PostgreSQL 포트 |
| database | POSTGRES_DB (환경 변수) | 데이터베이스명 |
| user | POSTGRES_USER (환경 변수) | 사용자명 |
| password | POSTGRES_PASSWORD (환경 변수) | 비밀번호 |

**사용 방법**: `psycopg2.pool.SimpleConnectionPool()` 생성 후 `getconn()`, `putconn()` 메서드로 연결 관리
