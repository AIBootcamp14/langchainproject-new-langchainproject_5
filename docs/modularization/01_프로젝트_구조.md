...
- **답변 표시 단계**: 채팅 화면에 답변 표시 후 용어 자동 추출 및 DB 저장, Streamlit의 자동 루프로 다시 입력창으로 이동

> **참고**: 성능 평가는 실시간 흐름이 아닌 별도 스크립트(`scripts/evaluate_answers.py`)로 실행되며, 답변 이후 Streamlit UI는 자동으로 입력창으로 복귀하여 연속 질문이 가능합니다.
