...
### 패턴 4: 하이브리드 용어집 검색 (glossary)

**컴포넌트 조합**:
```
VectorDB (유사도 검색) + RDBMS (키워드 검색) → 병합 → LLM (답변 생성)
```
...
사용자 질문: "BERT가 뭐야?"
    ↓
1. VectorDB 유사도 검색 (mode="hybrid")
    ├─ glossary_embeddings 컬렉션 검색
    └─ Top-3 용어 반환
    ↓
2. RDBMS 키워드 검색 (병렬 실행)
    ├─ glossary 테이블 ILIKE 검색
    ├─ term, definition, easy_explanation, hard_explanation 컬럼 검색
    └─ Top-3 용어 반환
    ↓
3. 결과 병합 및 중복 제거
    ├─ VectorDB 결과 + RDBMS 결과 합침
    └─ (term, definition) 조합으로 중복 제거
    ↓
4. LLM 답변 생성
    ├─ 난이도별(easy/hard) 설명 선택
    └─ 최종 답변 작성
```

**파일 위치**: `src/tools/glossary.py:322-423` (search_glossary 함수), `src/tools/glossary.py:428-510` (glossary_node)
...
if mode in ("hybrid", "vector"):
    ...
    # glossary_embeddings 컬렉션 유사도 검색
...
**검색 모드 3가지**:
- **hybrid**: VectorDB + RDBMS 병합 (정확도 최고)
- **vector**: VectorDB만 사용 (의미 기반 검색)
- **sql**: RDBMS만 사용 (키워드 매칭)

---

### 패턴 5: LLM + SQL 실행 (text2sql)
...
| 도구 | 컴포넌트 개수 | 주요 컴포넌트 | DB 접근 횟수 | LLM 호출 횟수 | 복잡도 | 하이브리드 |
|------|--------------|--------------|-------------|--------------|--------|-----------|
...
| **glossary** | 3개 | VectorDB + RDBMS + LLM | 2회 (hybrid 모드) | 1회 | 중간 | ✅ |
...
4. **glossary**: VectorDB + RDBMS + LLM (하이브리드 용어 검색)
5. **text2sql**: LLM + RDBMS (자연어 → SQL)
